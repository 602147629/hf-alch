...
var HFcLoadTm = {
		...
		statguide: function(step){
			if(arrayObj[step] != 1){
				arrayObj[step] = 1;
				var url = 'log/report';
				var param = {'type':'newuser', 'isNew':HFcLoadTm.isNewUser, 'step':step};
				HFcLoadTm.sendReq(url, param);
			}
		},
		flashloadlog: function(step) {
			var url = 'log/report';
			var param = {'type':'flashloadlog', 'isNew':HFcLoadTm.isNewUser, 'step':step};
			HFcLoadTm.sendReq(url, param);
		},
