...
class MixController extends Hapyfish2_Controller_Action_Api
{
    ...
    {
        $uid = $this->uid;
        $furnaceId = $this->_request->getParam('mixId', 0);
        $furnaceCid = $this->_request->getParam('cid');
        ...
        //$furnaceId = substr($furnaceId, 0, -2);
        $status = Hapyfish2_Alchemy_Bll_Mix::completeMix($uid, $furnaceId, $furnaceCid);
    ...
    public function expandfurnaceAction()
    {
        $uid = $this->uid;
        $furnaceCid = $this->_request->getParam('cid');
        $expandNum = $this->_request->getParam('num');
        

        $key = 'expandfurnace:' . $uid;
        $lock = Hapyfish2_Cache_Factory::getLock($uid);
        ...
        $status = Hapyfish2_Alchemy_Bll_Mix::expandFurnace($uid, $furnaceCid, $expandNum);
