...
class OpenApi_Kaixin_Kaixin
{
    ...
    {        
        ...
        $sex = isset($data['gender']) ? $data['gender'] : -1;
        //开心 0(男)1(女)统一为0(女)1(男)
        if ($sex == 1) {
            $gender = 0;
        } else if ($sex == 0) {
            $gender = 1;
    ...
    public function usersGetInfo()
    {
        ...
        if ($data['httpcode'] == 200) {
        	...
        	info_log(json_encode($data) . ':' . $this->access_token, 'openapi.usersGetInfo');
        }
    ...
    public function friendGetAppFriends()
    {
        ...
        //去重，理论上不需要，但可能平台出错会出现重复
        $fids = array_unique($fids);
        return $fids;
    }
    
    public function appGetInvitedIds($uid = null)
    {
    	if (!$uid) {
    		$uid = $this->user_id;
    	}
        ...
        $index = 0;
        while($index <= 20) {
        	$data = $this->app_invited($uid, $index, $pageNum);
        	if ($data['httpcode'] != 200) {
        		if ($data['httpcode'] != 304) {
        			info_log(json_encode($data) . ':' . $this->access_token, 'openapi.appGetInvitedIds');
        		}
        		return null;
        	}
        	if (!empty($data['response']['statuses'])) {
