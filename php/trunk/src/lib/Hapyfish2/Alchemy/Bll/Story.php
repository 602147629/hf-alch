...
class Hapyfish2_Alchemy_Bll_Story
{
	...
	public static function startStory($uid, $id, $isTest = false)
	{
		...
		if ($ok) {
			if ( $storyVo['coin'] > 0 ) {
				Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $storyVo['coin'],18);
			}
			if ( $storyVo['gem'] > 0 ) {
				$gemInfo = array('gem' => $storyVo['gem']);
				Hapyfish2_Alchemy_Bll_Gem::add($uid, $gemInfo,16);
			}
		...
		$newHelpId = 0;
		//剧情触发新手引导：设置角色，装备，布阵
		$storyTriHelp = array('291' => 22,
							  '41'  => 2,
							  '351' => 10,
							  '331' => 18,
							  '371' => 20,
							  '391' => 3);
		if ( isset($storyTriHelp[$id]) ) {
			$newHelpId = $storyTriHelp[$id];
		}
		if ( $newHelpId > 0 ) {
		...
		if ( $id == 261 ) {
			...
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'preLoadSwf', $preLoadSwf);
		}
		Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'story', $storyVo);
		
		//用户操作记录-首次播放剧情记录
        $actionAry = array('type' => 2, 'cid' => $id);
