...
class Hapyfish2_Alchemy_Bll_Task
{
                ...
                foreach ($aryCondition as $condi) {
                    ...
                    if ($cInfo) {
                        ...
                        $conditionVo[] = array(
			            	...
			            	'kind' => $cInfo['kind'],
			            	'sceneId' => $cInfo['scene_id'],
			            	'targetId' => $cInfo['target_id'],
			            	'node' => $cInfo['node'],
                        	'taskType' => $cInfo['condition_type']
                        );
                    }
                }
            }
    ...
    public static function getCurTaskList($uid)
	{
        ...
        if ($openTask['list']) {
            ...
            foreach ($openTask['list'] as $tid) {
            	$taInfo = self::genTaskVo($uid, $tid, null, $taskProcAll, $readStatus);
            	if(!empty($taInfo)){
            		$aryTaskVo[] = $taInfo;
            	}
            }
	...
	public static function completeTask($uid, $tid, $isComplete=1)
	{
		...
		if (!$isComplete) {
			...
			Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo,13);
		}
		...
		//任务解锁游戏内按钮
		if ( $tid == 431 ) {
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'worldMap');
		}
		else if ( $tid == 131 ) {
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'popUpUisprite');
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'goHome');
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'goVillage');
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'TreasureBox');
		}
		else if ( $tid == 401 ) {
			//Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'roleWork');
		}
		else if ( $tid == 581 ) {
			Hapyfish2_Alchemy_Bll_Help::unlockFunc($uid, 'CashCow');
		}
		
		//用户操作记录-完成任务id
		$actionAry = array('type' => 4, 'cid' => $tid);
