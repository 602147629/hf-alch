...
class Hapyfish2_Alchemy_Bll_Payment
{
    ...
    public static function completeOrder($uid, $orderid, $param = array())
    {
	    ...
	    $orderid = $order['orderid'];
	    $gold = (int)$order['gold'];
	    $ok = Hapyfish2_Alchemy_Bll_Gem::add($uid, array('gem' => $gold, 'type' => 101),14);
       	...
       	if ($include != null && !empty($include)) {
       		foreach ($include as $item) {
       			if ($item['cid'] > 0 && $item['num'] > 0) {
       				if ($item['cid'] == 1) {
       					Hapyfish2_Alchemy_Bll_Gem::add($uid, array('gem' => $item['num'], 'type' => 102),15);
       					$content[] = $item['num'].'宝石';
       				} else if ($item['cid'] == 2) {
       					Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $item['num'],16);
       					$content[] = $item['num'].'金币';
       				}
       				else {
