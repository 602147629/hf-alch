...
class Hapyfish2_Alchemy_Bll_Card
{
	...
	public static function useCard($uid, $cid, $roleId, $num)
	{
        ...
        if ( $smallType == 17 ) {
			...
			if ( in_array($cid, array(111, 212, 2112, 2212, 2811, 2911)) ) {
				...
				$result = self::expCard($uid, $cid, $cardInfo, $roleId, $num);
			}
			//急救箱
			...
			else if (in_array($cid, array(3001115))) {
				$result = self::addHireScoreCard($uid, $cid, $cardInfo, $num);
			}
		}
		else if ( $type == 2 ) {
	 ...
	 *
	 */
	public static function expCard($uid, $cid, $cardInfo, $roleId, $num)
	{
		if ( $roleId == 0 ) {
			return -351;
			//$userMercenary = Hapyfish2_Alchemy_HFC_FightAttribute::getInfo($uid);
	 ...
	 */
	  ...
	  * @param int $num
	  */
	public static function addHireScoreCard($uid, $cid, $cardInfo, $num)
	{
		//不同道具，加血量不同
		if ( $cid == 3001115 ) {
			$addScore = 100;
		...
		if ( $maxScore <= $userHireData['score'] ) {
			return -802;
		}
