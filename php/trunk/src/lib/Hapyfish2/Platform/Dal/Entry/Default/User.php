...
class Hapyfish2_Platform_Dal_Entry_Default_User
{
     ...
     *
     *
     * @return Hapyfish2_Platform_Dal_Entry_Default_User
     */
    ...
    public function add($user)
    {
        ...
        $create_time = time();
        $tbname = $this->getTableName($uid);
        $sql = "INSERT INTO $tbname (uid, puid, name, gender, figureurl, create_time) VALUES" . '(' . $uid . ',' . $puid . ',' . $name . ',' . $gender . ',' . $figureurl . ',' . $create_time . ')' . ' ON DUPLICATE KEY UPDATE ' . 'puid=' . $puid . ',name=' . $name . ',gender=' . $gender . ',figureurl=' . $figureurl;
