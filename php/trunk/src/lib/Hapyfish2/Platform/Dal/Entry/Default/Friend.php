...
class Hapyfish2_Platform_Dal_Entry_Default_Friend
{
     ...
     *
     *
     * @return Hapyfish2_Platform_Dal_Entry_Default_Friend
     */
    ...
    public function add($uid, $fids, $count)
    {
        ...
        $wdb = $db['w'];
        $fids = $wdb->quote($fids);
        $sql = "INSERT INTO $tbname (uid, fids, count) VALUES" . '(' . $uid . ',' . $fids . ',' . $count . ')' . ' ON DUPLICATE KEY UPDATE ' . 'fids=' . $fids . ',count=' . $count;
