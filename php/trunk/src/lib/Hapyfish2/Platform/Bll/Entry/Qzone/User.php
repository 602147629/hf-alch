...
class Hapyfish2_Platform_Bll_Entry_Qzone_User
{
    ...
    public static function getMultiUser($fids)
    {
        $info = array();
        foreach ($fids as $fid) {
            $user = Hapyfish2_Platform_Cache_MultipleEntry_Qzone_User::getUser($fid);
            if ($user && $user['puid']) {
                $info[] = array(
                    'uid' => $user['uid'], 
                    'name' => $user['name'], 
    ...
    {
        //get
        $old = Hapyfish2_Platform_Cache_MultipleEntry_Qzone_User::getUser($uid);
        if ($old) {
        ...
        $res = Hapyfish2_Platform_Cache_MultipleEntry_Qzone_User::updateUser($uid, $user, $savedb);
        if ($res) {
            $hc = Hapyfish2_Cache_HighCache::getInstance();
            $key = 'p:qzone:u:' . $uid;
            $data = array(
    ...
    {
        $res = Hapyfish2_Platform_Cache_MultipleEntry_Qzone_User::addUser($user);
        if ($res) {
            $hc = Hapyfish2_Cache_HighCache::getInstance();
            $uid = $user['uid'];
            $key = 'p:qzone:u:' . $uid;
