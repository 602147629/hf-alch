...
class Hapyfish2_Platform_Bll_Entry_Default_Friend
{
    ...
    public static function getFriend($uid)
    {
        ...
        $data = $hc->get($key);
        if (! $data) {
            $data = Hapyfish2_Platform_Cache_Entry_Default_Friend::getFriend($uid);
            if ($data) {
                $hc->set($key, $data);
            }
        }
    ...
    public static function updateFriend($uid, $fids, $highcache = false)
    {
        $count = count($fids);
        $fids = join(',', $fids);
        $res = Hapyfish2_Platform_Cache_Entry_Default_Friend::updateFriend($uid, $fids, $count);
        if ($res) {
            $data = array(
                'uid' => $uid, 
                'fids' => $fids, 
    ...
    public static function addFriend($uid, $fids, $highcache = false)
    {
        $count = count($fids);
        $fids = join(',', $fids);
        $res = Hapyfish2_Platform_Cache_Entry_Default_Friend::addFriend($uid, $fids, $count);
        if ($res) {
            $data = array(
                'uid' => $uid, 
                'fids' => $fids, 
