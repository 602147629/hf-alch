<?php
define('SERVER_ID', '1001');

define('APP_DIR', ROOT_DIR . DIRECTORY_SEPARATOR . 'app');
define('CONFIG_DIR', APP_DIR . DIRECTORY_SEPARATOR . 'config');
define('MODULES_DIR', APP_DIR . DIRECTORY_SEPARATOR . 'modules');
define('LIB_DIR', ROOT_DIR . DIRECTORY_SEPARATOR . 'lib');
define('DOC_DIR', ROOT_DIR . DIRECTORY_SEPARATOR . 'www');
define('LOG_DIR', '/data/alchemy/logs/debug');
define('STAT_LOG_DIR', '/data/alchemy/logs/stat');
define('TEMP_DIR', ROOT_DIR . DIRECTORY_SEPARATOR . 'temp');
define('SMARTY_TEMPLATES_C', ROOT_DIR . DIRECTORY_SEPARATOR . 'templates_c');

define('APP_ID', '100669243');
define('APP_KEY', '100669243');
define('APP_SECRET', 'f566c2e7fb092de980dc0a3d1d50eb68');
define('APP_NAME', '炼金大冒险');

define('PLATFORM', 'qzone');
define('PLATFORM_ENTRY_ID', 2);
define('APP_SERVER_TYPE', 1); //1 正服  2 测服 3 开发服
define('PREX_APC', 'alchemy:qq');
define('DATABASE_NODE_NUM', 4);
define('MEMCACHED_NODE_NUM', 10);
define('PUID_HASH', 1);
define('ENABLE_STATIC_GZ', false);
define('ENABLE_HFC', false);
define('ENABLE_DEBUG', true);
define('ENABLE_FIGHT_DB_LOG', true);
define('BOTFRIEND', '10017,10029');//公共好友

define('HOST', 'http://app100669243.qzone.qzoneapp.com');
define('STATIC_HOST', 'http://app100669243.imgcache.qzoneapp.com/app100669243');

define('SEND_ACTIVITY', true);
define('SEND_MESSAGE', false);

define('APP_STATUS', 1);
define('APP_STATUS_DEV', 0);

define('ECODE_NUM', 4);