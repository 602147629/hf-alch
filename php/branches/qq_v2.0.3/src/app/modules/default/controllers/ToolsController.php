...
//	}
    ...
    public function addgemAction()
    {
    	...
    	$ok = Hapyfish2_Alchemy_Bll_Gem::add($uid, $gemInfo,4);
    	if ( $ok ) {
	    	echo 'OK';
	    	exit;
    	}
    ...
    public function addcoinAction()
    {
    	...
    	$ok = Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $coin,1);
    	if ( $ok ) {
	    	echo 'OK';
	    	exit;
    	}
...
//    	$uid = $this->uid;
    ...
    public function sendcoinAction()
    {
    	...
    	foreach($userlist as $uid){
    		$num += 1;
    		$ok = Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, 5000,1);
    ...
    public function clearadditionAction()
    {
    	...
    	print_r(Hapyfish2_Alchemy_Cache_Vip::getVipAddition());
    	exit;
    }
...
//		Hapyfish2_Alchemy_Bll_EventGift::checkGift($uid,$num);
    ...
    public function sendpackageAction()
    {
    	...
    	foreach($userlist as $uid){
    		$num += 1;
    		$ok = Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, 30000,1);
    ...
    public function clearuserhelltowerAction()
    {
    	$uid = $this->_request->getParam('uid', -1);
		...
		$userwater['refreshTime'] = 0;
		$userAbyss['refreshTime'] = 0;
		if($max){
			$userwater['max'] = $max;
			$userAbyss['max'] = $max>30?30:$max;
		}
	   Hapyfish2_Alchemy_Cache_Helltower::updateUserWaterStatus($uid, $userwater);
	   Hapyfish2_Alchemy_Cache_Helltower::updateUserAbyssStatus($uid, $userAbyss);
	   echo "ok";
	   exit;
    }
    
    public function cleardmAction()
    {
    	$key = 'a:u:config:dm';
        $cache = Hapyfish2_Alchemy_Cache_Dm::getBasicMC();
        $cache->delete($key);
        echo "ok";
        exit;
    }
    ...
    {
    	$key = 'project:appserver:list';
    	$cache = Hapyfish2_Project_Cache_AppServer::getBasicMC();
    	$cache->delete($key);
    	echo "ok";
    	exit;
    }
    
    public function jumpwaterAction()
    {
    	$uid = $this->_request->getParam('uid', -1);
    	$max = $this->_request->getParam('num');
    	$userwater = Hapyfish2_Alchemy_Cache_Helltower::getUserWaterStatus($uid);
		$userwater['current'] = $max;
	   	Hapyfish2_Alchemy_Cache_Helltower::updateUserWaterStatus($uid, $userwater);
	   echo "ok";
	   exit;
    }
    
    public function jumpabyssAction()
    {
    	$uid = $this->_request->getParam('uid', -1);
    	$max = $this->_request->getParam('num');
