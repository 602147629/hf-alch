...
class PayController extends Hapyfish2_Controller_Action_Page
{
	...
	public function shippedAction()
	{
		...
		$data = Hapyfish2_Platform_Bll_QqpayByToken::shipped($uid,$token);
	...
	{
		$token = $this->_request->getParam('token');
		$result = array('status' => -200, 'content' => 'serverWord_200');
    	header("Cache-Control: no-store, no-cache, must-revalidate");
    	echo json_encode(array('result' => $result,'token'=>$token));
    	exit;
	}
}