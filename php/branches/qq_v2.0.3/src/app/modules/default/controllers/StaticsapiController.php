...
class StaticsapiController extends Zend_Controller_Action {
    ...
    public function coinAction() {
        ...
        $log = Hapyfish2_Stat_Bll_CoinStatistics::getDayCoinRecord($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }
    /**
     *  每日赠送钻石  统计
     */
    public function senddiamondAction(){
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_SendDiamond::getSendDiamondRecord($day);
        $data = array('data' => $log);
        $this->echoResult($data);    
    }
    //客户端加载时间day range
    ...
    public function useractionAction() {
        ...
        $log = Hapyfish2_Stat_Bll_UserAction::listData($day1, $day2);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    //用户操作记录day
    public function useractiondayAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_UserAction::getDay($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    //用户操作记录-每小时
    public function useractionhourAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_UserAction::getHour($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }
    ...
    public function CoinStatistics() {
        ...
        $log  = Hapyfish2_Stat_Dal_CoinStatistics::getDayCoinRecord($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }
    ...
    public function userhelpAction() {
        ...
        $log = Hapyfish2_Stat_Bll_NewUser::getUserHelp($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }
    ...
    //付费等级
    function getpaylevelAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_PayLevel::getPayLevel($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    //购买等级
    function getbuylevelAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_PayLevel::getBuyLevel($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    //消费等级
    function getgemlevelAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd", strtotime("-1 day"));
        }

        $log = Hapyfish2_Stat_Bll_PayLevel::getGemLevel($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    function getbuyitemAction() {
        $log = Hapyfish2_Stat_Bll_PayLevel::getBuyItem();
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    function getuserhourAction() {
        $day = $this->_request->getParam('day');
        if (empty($day)) {
            $day = date("Ymd");
        }
        $log = Hapyfish2_Stat_Bll_StatHour::getUserHour($day);
        $data = array('data' => $log);
        $this->echoResult($data);
    }

    public function getqqpaylistAction()
    {
    	$day = $this->_request->getParam('day', 'all');
    	if (empty($day)) {
    		$day = date("Ymd");
    	}
    	if ( $day == 'all' ) {
    		$log = Hapyfish2_Stat_Bll_Qzonepay::getAll();
    	}
    	else {
    		$log = Hapyfish2_Stat_Bll_Qzonepay::getDay($day);
    	}
    	$data = array('data' => $log);
    	$this->echoResult($data);
