<?php

class Hapyfish2_Alchemy_Cache_User
{
	public static function isAppUser($uid)
    {
        $key = 'a:u:isapp:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $data = $cache->get($key);

        if ($data === false) {
			if ($cache->isNotFound()) {
				$levelInfo = Hapyfish2_Alchemy_HFC_User::getUserLevel($uid);
				if (!$levelInfo) {
					return false;
				} else {
					$data = 'Y';
					$cache->set($key, $data);
					return true;
				}
			} else {
				return false;
			}
        }

        if ($data == 'Y') {
        	return true;
        } else {
        	return false;
        }
    }

    public static function setAppUser($uid)
    {
        $key = 'a:u:isapp:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $cache->set($key, 'Y');
    }

    public static function isSinglePointLogin($uid, $skey)
    {
        $key = 'a:u:hfskey:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $savedSkey = $cache->get($key);
        if (empty($savedSkey)) {
            return false;
        }
        if ($savedSkey != $skey) {
            return false;
        }
        return true;
    }

    public static function setLoginUserSkey($uid, $skey)
    {
        $key = 'a:u:hfskey:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $cache->set($key, $skey);
        return;
    }

    public static function getLoginUserSkeyWatch($uid)
    {
        $key = 'a:u:hfskeywch:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $skey = $cache->get($key);
        return $skey;
    }

    public static function setLoginUserSkeyWatch($uid, $skey)
    {
        $key = 'a:u:hfskeywch:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $cache->set($key, $skey, 86400);
        return;
    }
    
	public static function getCreateTime($uid)
    {
        $key = 'a:u:createtm:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $data = $cache->get($key);

        if ($data === false) {
			$dalUser = Hapyfish2_Alchemy_Dal_User::getDefaultInstance();
			$data = $dalUser->getCreateTime($uid);
			if (!$data) {
				return false;
			} else {
				$cache->set($key, $data, 86400);
			}
        }
        return $data;
    }
    
    public static function getAccess($uid)
    {
    	$key = 'a:u:access:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $data = $cache->get($key);
		$date = date('Ymd');
        if ($data === false || $data['date'] != $date) {
        	$data = array(
        		'list'=>array(),
        		'date'=>$date
        	);
			
        }
        return $data;
    }
    
    public static function updateAccess($uid, $data)
    {
    	$key = 'a:u:access:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $cache->set($key, $data);
    }

    /**
     * 今天已经使用的免费传送次数
     * @param int $uid
     * @return number
     */
    public static function getTodayUsedFreeTransportCnt($uid)
    {
    	$key = 'a:u:todaytranscnt:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 0;
    	}
    	return $data;
    }

    /**
     * 更新今天已经使用的免费传送次数
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function updateTodayFreeTransportCnt($uid, $data)
    {
    	$key = 'a:u:todaytranscnt:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data, 86400);
    	return;
    }

    /**
     * 建筑升级冷却结束时间
     * @param int $uid
     * @return number
     */
    public static function getBuildingCdTm($uid)
    {
    	$key = 'a:u:buildcdtm:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 0;
    	}
    	return $data;
    }
    
    /**
     * 更新建筑升级冷却结束时间
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function updateBuildingCdTm($uid, $data)
    {
    	$key = 'a:u:buildcdtm:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data, 86400);
    	return true;
    }

    /**
     * 玩家是否第一次培养佣兵
     * @param int $uid
     * @return number
     */
    public static function isFirstStrengthen($uid)
    {
    	$key = 'a:u:isfirststrth:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }
    
    /**
     * 第一次培养佣兵
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function setFirstStrengthen($uid, $data = 'N')
    {
    	$key = 'a:u:isfirststrth:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data);
    }
    
    public static function getUserSpAdd($uid)
    {
    	$key = 'a:u:sp:add:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $data = $cache->get($key);
		$date = date('Ymd');
        if ($data === false || $data['date'] != $date) {
        	$data = array(
        		'list'=>array(0,0,0,0),
        		'date'=>$date
        	);
        }
        return $data;
    }
    
    public static function updateUserSpAdd($uid,$data)
    {
    	$key = 'a:u:sp:add:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getMC($uid);
        $cache->set($key,$data);
    }

    /**
     * 玩家是否第一次修复新手任务
     * @param int $uid
     * @return number
     */
    public static function isFirstRecoupTaskForNewHelp($uid)
    {
    	$key = 'a:u:isfirstrecouptfnh:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }
    
    /**
     * 第一次修复新手任务
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function setFirstRecoupTaskForNewHelp($uid, $data = 'N')
    {
    	$key = 'a:u:isfirstrecouptfnh:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data);
    }
        /**
     * 今天是否领可以领取QQ黄钻每日礼包
     * @param int $uid
     * @return number
     */
    public static function canGetTodayQQYellowVipGift($uid)
    {
    	$key = 'a:u:todayqqyelvipgift:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }

    /**
     * 玩家是否第一次修复酒馆人气值
     * @param int $uid
     * @return number
     */
    public static function isFirstRecoupHireScore($uid)
    {
    	$key = 'a:u:isfrtrcphirescore:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }
    
    /**
     * 第一次修复酒馆人气值
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function setFirstRecoupHireScore($uid, $data = 'N')
    {
    	$key = 'a:u:isfrtrcphirescore:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data);
    }
    
    /**
     * 更新今天领取QQ黄钻每日礼包信息
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function updateTodayQQYellowVipGift($uid, $data)
    {
    	$key = 'a:u:todayqqyelvipgift:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data, 86400);
    	return;
    }

    /**
     * 今天是否领可以领取QQ黄钻新手礼包
     * 
     * @param int $uid
     * @return number
     */
    public static function canGetGuideQQYellowVipGift($uid)
    {
    	$key = 'a:u:guideqqyelvipgift:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }
    
    /**
     * 更新今天领取QQ黄钻新手礼包信息
     * 
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function updateGuideQQYellowVipGift($uid, $data)
    {
    	$key = 'a:u:guideqqyelvipgift:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data, 86400);
    	return;
    }
    
    /**
     * 玩家是否第一次记录“用户操作记录”log
     * @param int $uid
     * @return number
     */
    public static function isFirstActionLog($uid)
    {
    	$key = 'a:u:isfirstactionlog:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$data = $cache->get($key);
    	if ( $data === false ) {
    		$data = 'Y';
    	}
    	return $data;
    }
    
    /**
     * 第一次记录“用户操作记录”log
     * @param int $uid
     * @param int $data
     * @return number
     */
    public static function setFirstActionLog($uid, $data = 'N')
    {
    	$key = 'a:u:isfirstactionlog:' . $uid;
    	$cache = Hapyfish2_Cache_Factory::getMC($uid);
    	$cache->set($key, $data);
    }
    
    
}