...
class Hapyfish2_Alchemy_Dal_EventGift {
    ...
    public function getYellowGift()
    {
    	$sql = "select `type`,`level`,nextLevel,awards from alchemy_yellow_gift order by `level`";
    	$db = $this->getDB();
    	$rdb = $db['r'];
        return $rdb->fetchAll($sql);
    }
    ...
    public function insertYgift($data)
    {
    	$db = $this->getDB();
    	$wdb = $db['w'];
    	$sql = "INSERT INTO alchemy_yellow_gift (level, nextLevel, `awards`, `type`) VALUES(:level, :nextLevel, :awards, :type) ON DUPLICATE KEY UPDATE nextLevel=:nextLevel,awards=:awards";
    	return $wdb->query($sql, array('level'=>$data['level'], 'nextLevel'=>$data['nextLevel'], 'type'=>$data['type'],'awards'=>$data['awards']));
    }
    
	public function deletey($data)
    {
    	$db = $this->getDB();
    	$wdb = $db['w'];
    	$sql = "delete from alchemy_yellow_gift where `level`=:level and `type`=:type";
    	return $wdb->query($sql, array('level'=>$data['level'],'type'=>$data['type']));
    }
}
