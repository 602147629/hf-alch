...
/**********************************************************************************************/
    ...
    public function getHelltowerShop()
    {
    	$sql = "SELECT `cid`,`price`,`vip_price`,`needWater`,`needAbyss`,`order` FROM alchemy_helltower_shop order by `order`";
        $db = $this->getDB();
        $rdb = $db['r'];
        return $rdb->fetchAssoc($sql);
    }
    
    public function getHelltowerConfig()
    {
    	$sql = "SELECT `id`,`awards`,`type`,`monster`, `monster_stations` FROM alchemy_helltower_config";
     ...
     * 竞技场排名奖励表
    */
    public function getArenaPrize()
    {
    	$sql = "SELECT * FROM alchemy_arena_prize ORDER BY rank";
    	$db = $this->getDB();
    	$rdb = $db['r'];
    	return $rdb->fetchAssoc($sql);
    }
    ...
    */
     ...
     * 酒馆积分 对应的 佣兵星级 信息
     */
    public function getHireScoreRpList()
    {
    	$sql = "SELECT * FROM alchemy_hire_rp_score";
    	$db = $this->getDB();
    	$rdb = $db['r'];
    	return $rdb->fetchAssoc($sql);
    }
     ...
     * 佣兵星级 对应的酒馆刷新信息
     */
    public function getHireRpList()
    {
    	$sql = "SELECT * FROM alchemy_hire_rp";
    	$db = $this->getDB();
    	$rdb = $db['r'];
    	return $rdb->fetchAll($sql);
    }
     ...
     * 建筑升级tips
     */
    public function getBuildUpgradeList()
    {
    	$sql = "SELECT * FROM alchemy_level_build ORDER BY `order`";
    	$db = $this->getDB();
    	$rdb = $db['r'];
    	return $rdb->fetchAll($sql);
    }
     ...
     * QQ黄钻等级 每日礼包
    */
    public function getQQYellowVipGiftList()
    {
    	$sql = "SELECT * FROM alchemy_qq_yellowvip_dailygift";
    	$db = $this->getDB();
    	$rdb = $db['r'];
    	return $rdb->fetchAssoc($sql);
    }
