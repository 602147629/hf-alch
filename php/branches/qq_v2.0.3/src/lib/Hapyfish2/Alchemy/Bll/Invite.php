...
class Hapyfish2_Alchemy_Bll_Invite
{
	public static function add($inviteUid, $newUid, $time = null)
	{
		...
		if ($ok) {
			...
			//500金币
			Hapyfish2_Alchemy_HFC_User::incUserCoin($inviteUid, 1000,11);
			Hapyfish2_Alchemy_Event_Bll_Guild::addInvitePiont($inviteUid,1);
			$invite = Hapyfish2_Alchemy_HFC_User::getTotalInvite($inviteUid);
			$invite += 1;
			Hapyfish2_Alchemy_HFC_User::addTotalInvite($inviteUid, $invite);
			//1瓶大行动力药剂
