		...
		foreach ($fids as $fid) {
			...
			if ($userInfo && $avatar) {
				...
				$uVo = array(
					...
					'vipState' => $vipInfo['vipStatus']
				);
				if ( PLATFORM == 'sinaweibo' ) {
					$uVo['sinaVip'] = Hapyfish2_Platform_Cache_Entry_SinaWeibo_User::getUserV($fid);
				}
				else if ( PLATFORM == 'pengyou' || PLATFORM == 'qzone' ) {
					$userYellowInfo = Hapyfish2_Platform_Bll_User::getYellowInfo($fid);
