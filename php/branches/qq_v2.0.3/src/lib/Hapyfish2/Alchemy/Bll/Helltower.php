...
//		}
    ...
    public static function finishQQWaterSweep($uid)
    {
    	$userwater = Hapyfish2_Alchemy_Cache_Helltower::getUserWaterStatus($uid);
    	if($userwater['current'] > $userwater['max']){
    		return -200;
    	...
    	$needGem = 10;
    	$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '水牢扫荡加速*水牢扫荡加速';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p014*'.$needGem.'*1';
    ...
    public static function revivalQQWaterDungeon($uid)
    {
    	$userRefresh = Hapyfish2_Alchemy_Cache_Helltower::getUserWaterRefiesh($uid);
    	if($userRefresh['refresh'] > 1){
       		$needGem = 10;
       	...
       	if($needGem > 0){
       		$goodsInfo = array();
	    	$goodsInfo['goodsmeta'] = '水牢复活*水牢复活';
	    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
	    	$goodsInfo['payitem'] = 'p012*'.$needGem.'*1';
			$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid,$goodsInfo);
			if($token){
				Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
   ...
   public static function revivalQQAbyssDungeon($uid)
    {
    	$userRefresh = Hapyfish2_Alchemy_Cache_Helltower::getUserAbyssRefiesh($uid);
    	if($userRefresh['refresh'] > 1){
       		$needGem = 10;
       	...
       	if($needGem > 0){
	       $goodsInfo = array();
	    	$goodsInfo['goodsmeta'] = '深渊复活*深渊复活';
	    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
	    	$goodsInfo['payitem'] = 'p013*'.$needGem.'*1';
			$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid,$goodsInfo);
			if($token){
				Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
