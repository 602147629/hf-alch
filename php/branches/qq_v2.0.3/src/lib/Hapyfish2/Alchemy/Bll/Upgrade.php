...
class Hapyfish2_Alchemy_Bll_Upgrade
{
	...
	public static function removeBuildingCd($uid)
	{
		...
		Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo,15);
	...
	public static function removeQqBuildingCd($uid)
	{
		$nowTm = time();
		//用户最后一次建筑升级时间
		$buildingCdTm = Hapyfish2_Alchemy_Cache_User::getBuildingCdTm($uid);
		...
		$needGem = $needCount * 2;
		$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '清除建筑CD*清除建筑CD';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p009*'.$needGem.'*1';
	...
	public static function homeUpgrade($uid)
	{
		...
		//扣除金币
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],13);
	...
	public static function tavernUpgrade($uid, $id)
	{
		...
		//扣除金币，声望
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],14);
	...
	public static function smithyUpgrade($uid)
	{
		...
		//扣除金币，声望
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],15);
	...
	public static function arenaUpgrade($uid)
	{
		...
		//扣除金币
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],16);
	...
	public static function trainingUpgrade($uid)
	{
		...
		$needs = json_decode($levelInfo['need_items']);
				...
				//扣除金币
				Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],17);
