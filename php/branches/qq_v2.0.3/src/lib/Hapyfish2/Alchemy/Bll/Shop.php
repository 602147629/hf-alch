...
class Hapyfish2_Alchemy_Bll_Shop
{
	...
	public static function buyOneItem($uid, $cid, $num)
	{
		...
		if ($needCoin > 0) {
			...
			Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,12);
		}
		else if ($needGem > 0) {
	        ...
	        $type = Hapyfish2_Alchemy_Bll_Gem::getGemType($cid);
                //vip购买 分别统计
                 if($cid >= 6115 && $cid <= 6315){
                      Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo, 18);
                } else{
                     Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo, 11);
                }
	}
	...
	public static function buyItem($uid, $buyItem, $userLevel)
	{
		foreach ( $buyItem as $item ) {
			if ( $item['coin'] > 0 ) {
				...
				Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo, 12);
			}
			if ( $item['itemType'] == 1 ) {
	...
	public static function saleItem($uid, $saleItem, $userLevel)
	{
		foreach ( $saleItem as $item ) {
			if ( $item['coin'] > 0 ) {
				...
				Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $saleCoin,17);
			}
	...
	public static function buyOneItemQq($uid, $cid, $num = 1)
	{
		$needCoin =	0;
		...
		if ( $itemInfo['buy_coin'] > 0 ) {
			...
			//report log,统计玩家商店购买物品信息
			$logger = Hapyfish2_Util_Log::getInstance();
			$logger->report('331', array($uid, $cid, $itemInfo['name'], $num, $needCoin, 0));
	        $logger->report('buyLevel', array($uid, $fightLevel, $itemInfo['cid'], $num));
		}
		if($itemInfo['buy_gem'] > 0){
			$goodsInfo = Hapyfish2_Platform_Bll_QqpayByToken::createItemInfo($cid,$num,$itemInfo);
//			info_log('goodInfo:'.json_encode($goodsInfo), 'pay');
			...
			if($token){
				...
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payNeedData', array());
			}
		}
	...
	public static function buyItemsQq($uid, $items)
	{
		$buyItem =	array();
		$needCoin = 0;
		...
		foreach ( $buyItem as $item ) {
			...
			if($item['gem']>0){
				$pointPay = true;
				$payItem[] = $item['cid'].'*'.$item['gem'].'*'.$item['count'];
			}
		}
		if($pointPay){
			$payitems = implode(';', $payItem);
			$goodsInfo = array();
	    	$goodsInfo['goodsmeta'] = '材料补齐*材料不齐';
	    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
	    	$goodsInfo['payitem'] = $payitems;
			$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid, $goodsInfo);
			if($token){
				Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
