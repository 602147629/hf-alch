...
//	    }
	...
	public static function check($uid, $openTask, $taskInfo, $chkComplete=1)
	{
		...
		if ($ok) {
			...
			$userRoleBefore = Hapyfish2_Alchemy_HFC_FightAttribute::getInfo($uid);
			...
			$userRole = Hapyfish2_Alchemy_HFC_FightAttribute::getInfo($uid);
			...
			if ( $userRoleBefore['level'] != $userRole['level'] ) {
                $openTask = Hapyfish2_Alchemy_HFC_TaskOpen::getInfo($uid);
			}
			...
			//用户操作记录-完成任务id
			$actionAry = array('type' => 4, 'cid' => $taskId);
