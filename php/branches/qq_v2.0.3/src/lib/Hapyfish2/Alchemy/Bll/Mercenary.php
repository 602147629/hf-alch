...
class Hapyfish2_Alchemy_Bll_Mercenary
{
	...
	public static function refreshHire($uid, $type, $fast = 0)
	{
		...
		$needGem = $needCoin = $hireType = $addScore = 0;
		...
		//金币刷新剩余次数判断
		if ( $hireType == 0 && $hireInfo['times'] < 1 ) {
			return -342;
		}
		...
		for ( $i=1;$i<4;$i++ ) {
			...
			/* if ( $i == 2 && $firstRefresh > 0 ) {
				$newHire = self::getNewHireByFirstRef($uid, $i, $hireList[$i], $firstRefresh, $curScoreRp);
				if ( !$newHire ) {
					$newHire = self::getNewHire($uid, $i, $hireList[$i], $hireType, $curScoreRp);
				}
			}
			else {
				//获取一个新的雇佣佣兵信息
		...
		//更新剩余次数,只有普通金币刷新的时候 才记录 剩余次数，其余类型刷新 不限制次数
		if ( $hireType == 0 ) {
			$hireInfo['times'] -= 1;
		...
		//酒馆等级信息
		$levelInfo = Hapyfish2_Alchemy_Cache_Basic::getTavernLevel($userTavernLevel['tavern_level'], TAVERN_ID);

		$maxScore = $levelInfo['max_score'];
		$userHireData['score'] = $userHireData['score'] > $maxScore ? $maxScore : $userHireData['score'];
	    ...
	    if ( $needCoin > 0 ) {
	    	Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,3);
	    }
	    if ( $needGem > 0 ) {
	    	...
	    	Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo, 3);
	    }
		...
		$logger->report('302', array($uid, $type, $hireType, $needGem));
		
		return 1;
	}
	public static function refreshQQHire($uid, $type, $fast = 0)
	{
		if ( !in_array($type, array(0,1,2,3))) {
			return -200;
		}
		...
		$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '佣兵位刷新*佣兵位刷新';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p004*'.$needGem.'*1';
	...
	public static function hireMercenary($uid, $id)
	{
		...
		$userHireData = Hapyfish2_Alchemy_HFC_Hire::getHireData($uid);
		$userHireInfo = $userHireData['data'];
		$userCurScore = $userHireData['score'];
		
		$needScore = $hireInfo['coin'];
		if ( $userCurScore < $needScore ) {
			return -801;
		}
		/* 
		$userCoin = Hapyfish2_Alchemy_HFC_User::getUserCoin($uid);
		...
		//扣除酒馆积分
		if ( $needScore > 0 ) {
			$userHireData['score'] -= $needScore;
			$userHireData['score'] = $userHireData['score'] < 0 ? 0 : $userHireData['score'];
		...
		$logger = Hapyfish2_Util_Log::getInstance();
		$logger->report('301', array($uid, $userLevel, $selfMercenary['level'], $hireInfo['rp'], $hireInfo['job'], $hireInfo['element'], $needScore));
	...
	public static function completeHire($uid, $id, $npcId, $type = 1)
	{
		...
		if ( $type == 3 ) {
			...
			$ok = Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,4);
		}//使用酒道具
		else {
			$needCid = 1115;
			$needCount = 1;
	...
	public static function dismissMercenary($uid, $id)
	{
		...
		//解雇返还经验卷轴
		$expAdd = Hapyfish2_Alchemy_Bll_VipWelfare::getVipReturnExp($uid);
		$returnCount = floor($userMercenary['exp'] * $expAdd / 200);
		if ( $returnCount > 0 ) {
	...
	public static function replaceSkill($uid, $id, $idx, $cid)
	{
		...
		//是否已学习该技能
		$isExist = array_search($cid, $hasSkillList);
		if ( $isExist === false ) {
			return -807;
		}
	...
	public static function unlockSkill($uid, $id, $idx)
	{
		...
		if ( $needCoin > 0 ) {
			Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,5);
		}
		if ( $needGem > 0 ) {
			$userLevel = Hapyfish2_Alchemy_HFC_User::getUserLevel($uid);
			//扣除用户宝石
	...
	public static function unlockQqSkill($uid, $id, $idx)
	{
		if ( !in_array($idx, array(1,2,3,4,5)) ) {
			return -200;
		}
		...
		$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '解锁技能*解锁技能';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p005*'.$needGem.'*1';
		$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid,$goodsInfo);
		if($token){
			Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'token', $token['token']);
			$payNeedData = array('id'=>$id, 'idx'=>$idx);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payNeedData', $payNeedData);
		}
		return 1;
	}
	...
	public static function expandRoleLimit($uid, $payType)
	{
		...
		if ( $needCoin > 0 ) {
			Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,6);
		}
		if ( $needGem > 0 ) {
			$userLevel = Hapyfish2_Alchemy_HFC_User::getUserLevel($uid);
			//扣除用户宝石
			...
			Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo,5);
		}
	...
	public static function expandQqRoleLimit($uid, $payType)
	{
		//用户当前最大佣兵数
		$userMaxCount = Hapyfish2_Alchemy_HFC_User::getUserMaxMercenaryCount($uid);
		...
		$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '扩展佣兵位*扩展佣兵位';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p006*'.$needGem.'*1';
		$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid, $goodsInfo);
		if($token){
			Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
	...
	public static function resumeHp($uid)
	{
        ...
        if ( $needCoin > 0 ) {
			...
			Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,7);
        }
	...
	public static function roleUpgrade($uid)
	{
		...
		//扣除金币
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $levelInfo['need_coin'],8);
	...
	public static function resetRoleQuality($uid, $locks)
	{
		...
		try {
			...
			Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo,6);
		}
		catch ( Exception $e ) {
	...
	public static function resetRoleQqQuality($uid, $locks)
	{
		//$fieldAry = array('hp', 'mp', 'phy_att', 'phy_def', 'mag_att', 'mag_def', 'agility', 'crit', 'dodge', 'hit', 'tou');
		$fieldAry = array('1' => 'hp',
						'2' => 'mp',
		...
		//佣兵模型信息
		$goodsInfo = array();
    	$goodsInfo['goodsmeta'] = '重置主角属性*重置主角属性';
    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
    	$goodsInfo['payitem'] = 'p007*'.$needGem.'*1';
		$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid,$goodsInfo);
		if($token){
			Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'token', $token['token']);
			$payNeedData = array('locks'=>$locks);
			Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payNeedData', $payNeedData);
		}
		return 1;
...
/********************************************************************************************/
	 ...
	 */
	public static function getNewHire($uid, $id, $hire, $type, $rp)
	{
		$maxCount = 21;
	
		//随机星级,职业,元素属性:风火水
		//$rp = self::getRandRp($hire['level'], $type, $score);
		...
		//随机名字
		$modelInfo = array();
		$modelInfo = self::getRandModelWithName($modelInfo, $userNameList, $maxCount, $job, $hire, $rp);
		...
		//根据星级判断需要积分数
		$curScoreRp = Hapyfish2_Alchemy_Cache_BasicExt::getHireScoreRp($rp);
		$hire['coin'] = $curScoreRp['need_score'];
		$hire['feats'] = 0;
	 ...
	 * @param int $rp , 佣兵星级 
	 */
	public static function getRandModelWithName($modelInfo, $userNameList, $maxCount, $job, $hire, $rp)
	{
		$modelNameList = Hapyfish2_Alchemy_Cache_Basic::getMercenaryNameList($maxCount, $job, $rp);
	 ...
	 * @param int $score,当前酒馆积分数
	 */
	public static function getRandRp($level, $type, $score)
	{
...
//			$vip->sendUpAward($uid, $newLevel);
			...
			if($newLevel == 20){
				Hapyfish2_Alchemy_Bll_WorldMap::setWorldMapOpened($uid, 3001);
                      ...
                      //记录新注册用户 战斗等级 分布情况
                        ...
                        if($createTm >= mktime(0,0,0)){
                          $log = Hapyfish2_Util_Log::getInstance();
                          $log->report('newuserlevel', array($uid, $nowLevel));  //过去战斗等级
                          $log->report('newuserlevel', array($uid, $newLevel));  //最新的战斗等级
                        }
		}
		else {
	...
	public static function addWine($uid, $fid)
	{
		...
		$addExp = 1;
		if ( $addCoin > 0 ) {
			Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $addCoin,99);		
		}
		if ( $addExp > 0 ) {
	...
	public static function strengthenStart($uid, $id, $type)
	{
		...
		if ( $isFirstStr != 'Y' ) {
			if ( $type == 2 ) {
				...
				$needCoin = 100 * ( 1 + pow( floor(($userMercenary['level'] - 1) / 5), 2) );
				if ( $needCoin < 1000 ) {
					$needCoin = round($needCoin/10) * 10;
				}
				else if ( $needCoin < 10000 ) {
					$needCoin = round($needCoin/100) * 100;
				}
				else {
					$needCoin = round($needCoin/1000) * 1000;
				}
			}
		...
		//获取培养随机数据
		$strengthen = self::getStrengthen($userMercenary['job'], $userMercenary['level'], $type, $roleStrthen);
		...
		if ( $needCoin > 0 ) {
			Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,9);
		}
		if ( $needGem > 0 ) {
			$userLevel = Hapyfish2_Alchemy_HFC_User::getUserLevel($uid);
			//扣除用户宝石
	...
	public static function strengQQthenStart($uid, $id, $type)
	{
		if ( $id == 0 ) {
			//主角信息
        	$userMercenary = Hapyfish2_Alchemy_HFC_FightAttribute::getInfo($uid);
		...
		//report log,统计玩家佣兵培养信息-培养次数
			$logger = Hapyfish2_Util_Log::getInstance();
			$logger->report('305', array($uid, 0, 0, $userMercenary['level']));
		}else{
			$goodsInfo = array();
	    	$goodsInfo['goodsmeta'] = '佣兵培养*佣兵培养';
	    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
	    	$goodsInfo['payitem'] = 'p003*'.$needGem.'*1';
			$token = Hapyfish2_Platform_Bll_QqpayByToken::getToken($uid,$goodsInfo);
			if($token){
				Hapyfish2_Alchemy_Bll_UserResult::setYellow($uid);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payJs', $token['url_params']);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'token', $token['token']);
				$payNeedData = array('id'=>$id, 'type'=>$type);
				Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'payNeedData', $payNeedData);
			}
		}
		$roleVo = array('id' => $id,
	 ...
	 * @param array $roleStrthen,佣兵当前培养属性信息
	 */
	public static function getStrengthen($job, $level, $type, $roleStrthen)
	{
		//获取对应培养信息
		...
		$sStr = self::_getStrengthenValue($strengthen['min_str'], $strengthen['max_str'], $strengthen['k_str'], $m, $n, $roleStrthen['s_str']);
		$sDex = self::_getStrengthenValue($strengthen['min_dex'], $strengthen['max_dex'], $strengthen['k_dex'], $m, $n, $roleStrthen['s_dex']);
		$sMag = self::_getStrengthenValue($strengthen['min_mag'], $strengthen['max_mag'], $strengthen['k_mag'], $m, $n, $roleStrthen['s_mag']);
		$sPhy = self::_getStrengthenValue($strengthen['min_phy'], $strengthen['max_phy'], $strengthen['k_phy'], $m, $n, $roleStrthen['s_phy']);
		
		$strengthen = array('s_str' => $sStr,
							's_dex' => $sDex,
							's_mag' => $sMag,
							's_phy' => $sPhy);
	 ...
	 * 成长值计算公式
	 * 
	 * @param int $min，该培养模式下 最小值
	 * @param int $max，该培养模式下 最大值
	 * @param int $k，系数k
	 * @param int $m，系数m，默认 除以100
	 * @param int $n，系数n，默认 除以100
	 * @param int $cur，该属性当前培养值
	 * @return number
	 */
	private static function _getStrengthenValue($min, $max, $k, $m, $n, $cur)
	{
		$cur = $cur * 1000;
		
		$temp = ($cur + $k);
		if ( $temp > $min && $temp <= $max ) {
			$randMin = $min + $m/100 * ($cur - $min);
			$randMax = $cur + min( $n/100 * ($max - $cur), $k);
			$new = rand($randMin, $randMax);
			$new = $new > $max ? $max : $new;
		}
		else if ( $temp <= $min ) {
			$new = $cur + $k;
			...
			$randMin = max(($min + $m/100 * ($cur - $min)), $max);
			$randMax = $max;
			$new = rand($randMin, $randMax);
		}
		return round($new/1000);
	}
