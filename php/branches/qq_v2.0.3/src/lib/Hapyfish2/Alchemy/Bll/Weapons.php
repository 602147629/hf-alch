...
class Hapyfish2_Alchemy_Bll_Weapons
{
	...
	public static function strengthenequipment1($uid,$id1,$id2)
	{
        ...
        if ($ok) {
        	Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $needCoin,54);
	...
	public static function strengthenequipment($uid, $roleId, $wid, $useType = 0)
	{
		...
		Hapyfish2_Alchemy_HFC_User::decUserCoin($uid, $cost,18);
		if($strStatus){
	...
	public static function clearCoolTime($uid)
	{
		...
		if(PLATFORM == 'pengyou' || PLATFORM == 'qzone'){
			$goodsInfo = array();
	    	$goodsInfo['goodsmeta'] = '清除强化冷却*清除强化冷却';
	    	$goodsInfo['goodsurl'] = STATIC_HOST.'/alchemy/image/item/qpoint.jpg';
	    	$goodsInfo['payitem'] = 'p002*'.$cost.'*1';
			...
			Hapyfish2_Alchemy_Bll_Gem::consume($uid, $gemInfo, 17);
			$userStr['endtime'] = 0;
			$userStr['canStr'] = 1;
