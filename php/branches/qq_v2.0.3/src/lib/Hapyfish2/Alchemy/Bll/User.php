...
class Hapyfish2_Alchemy_Bll_User
{
	public static function getUserInit($uid)
	{
		...
		$viliageInfo = array(HOME_ID => (int)$userHomeLevel,
							 ...
							 TAVERN_CITY_ID => (int)$userTavernLevel['tavern_level'],
		...
		if( PLATFORM == 'sinaweibo' ){
			$uVo['sinaVip'] = Hapyfish2_Platform_Cache_Entry_SinaWeibo_User::getUserV($uid);
		}
		else if ( PLATFORM == 'pengyou' || PLATFORM == 'qzone' ) {
			$userYellowInfo = Hapyfish2_Platform_Bll_User::getYellowInfo($uid);
			
			$uVo['isYellow'] = (int)$userYellowInfo['is_yellow_vip'];
			$uVo['yellowLevel'] = $userYellowInfo['yellow_vip_level'];
			$uVo['isYearYellow'] = (int)$userYellowInfo['is_yellow_year_vip'];
		}
	...
	public static function initRole($uid, $id = 1)
	{
		...
		if ( $info['job'] == 1 ) {
			$data['skill_list'] = '[44622]';
		}
		else if ( $info['job'] == 2 ) {
			$data['skill_list'] = '[44722]';
		}
		else if ( $info['job'] == 3 ) {
			$data['skill_list'] = '[34022]';
		}
	...
	public static function checkLevelUp($uid)
	{
		...
		//根据当前经验，查询最大等级
		$nextLevelInfo = Hapyfish2_Alchemy_Cache_Basic::getUserNextLevelByExp($userExp);
...
//			Hapyfish2_Alchemy_HFC_User::updateUserSp($uid, $userSp);
			...
			if ($nextLevelInfo['coin'] > 0) {
				Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $nextLevelInfo['coin'],19);
			}
			//gem
			if ($nextLevelInfo['gem'] > 0) {
				$gemInfo = array('gem' => $nextLevelInfo['gem']);
				Hapyfish2_Alchemy_Bll_Gem::add($uid, $gemInfo,17);
			}
			//items
	...
	public static function updateLoginTime($uid)
	{
		...
		if ($todayTm > $loginInfo['last_login_time']) {
            ...
            Hapyfish2_Alchemy_Bll_Arena::resetUserArena($uid, $userScore);
            
            //QQ平台特殊化
            if ( PLATFORM == 'pengyou' || PLATFORM == 'qzone' ) {
            	//充值用户领取 QQ黄钻等级 每日礼包信息
				Hapyfish2_Alchemy_Cache_User::updateTodayQQYellowVipGift($uid, 'Y');
            }
		}
		...
		$lastLogin = date('YmdG',$loginInfo['last_login_time']);
		$nowlogin = date('YmdG',$newLoginInfo['last_login_time']);
		if($nowlogin != $lastLogin){
			$logger->report('userHour', array($uid, $joinTime));
		}
...
/***********************************************************/
    ...
    public static function getUserDmStatus($uid)
    {
    	$userStatus = Hapyfish2_Alchemy_HFC_User::getUserStatus($uid);
    	if($userStatus['status'] == 0){
    		$userStatus['status'] = 1;
    		Hapyfish2_Alchemy_HFC_User::updateUserStatus($uid,$userStatus);
    		Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'status', 1);
    		return 1;	
    	}
    	Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'status', 0);
    	return 1;	
    }
