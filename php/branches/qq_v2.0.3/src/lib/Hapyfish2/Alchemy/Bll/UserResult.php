...
class Hapyfish2_Alchemy_Bll_UserResult
{
	...
	private static $platformFeedList = array();

	public static function addField($uid, $name, $value, $remove = true)
	{
		if (self::$uid == $uid) {
			if ( $name == 'rolesChange' || $name == 'npcListChange' ) {
				if ( $remove ) {
	...
	public static function setYellow($uid)
	{
		if (self::$uid == $uid) {
			self::$yellow = 1;
		}
	...
	public static function addPlatformFeed($uid, $feed)
	{
		if (self::$uid == $uid) {
			self::$platformFeedList[] = $feed;
		}
	}
	
	public static function getPlatformFeed()
	{
		return self::$platformFeedList;
	}
	...
	public static function all()
	{
		...
		if (count(self::$platformFeedList) > 0) {
			$ret['feeds'] = self::$platformFeedList;
		}
		if(self::$yellow == 1){
			$rets = $ret;
			unset($rets['payJs']);
			unset($rets['result']);
			Hapyfish2_Alchemy_Cache_payUserResult::addPayResult(self::$uid, $rets);
			return array('payJs'=>$ret['payJs'],'token'=>$ret['token'],'result'=>$ret['result']);
		}
