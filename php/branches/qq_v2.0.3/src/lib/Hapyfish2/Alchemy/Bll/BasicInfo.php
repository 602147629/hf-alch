...
class Hapyfish2_Alchemy_Bll_BasicInfo
{
	...
	public static function getInitVo()
	{
		...
		//佣兵培养
		$resultInitVo['strengThenStaticVo'] = self::getStrengThenClass();
		$resultInitVo['dailyDiscountGift'] = Hapyfish2_Alchemy_Bll_SpecialShop::initstatic();
		
		//QQ平台特殊化
		if ( PLATFORM == 'pengyou' || PLATFORM == 'qzone' ) {
			$resultInitVo['qqDailyGifts'] = self::getQQYellowVipDailyGift();
		}
...
//		$resultInitVo['autoUseList'] = self::getAutoList();
	...
	public static function getGameData()
	{
		...
		$info['helpEnabled'] = 0;
		$info['abyssDungeonDoorId'] = 7361;
		$info['waterDungeonDoorId'] = 7344;
		$info['hellTowerSceneId'] = 3001;
		$info['hireCountMaxList'] = self::getHireCountMaxList();
		return $info;
	}
	...
	public static function getBuildUpgradeBasic()
	{
		$info = array();
		$data = Hapyfish2_Alchemy_Cache_BasicExt::getBuildUpgradeList();
		foreach ($data as $item) {
			$info[] = array(
					'id' => (int)$item['id'],
					'name' => $item['name'],
					'maxLevel' => $item['max_level'],
					'levelTips' => $item['level_tips'],
					'lockName' => $item['lock_name']
			);
		}
		return array('buildInfos' => $info);
	}
	...
	public static function getMapCopyVerList()
	{
        ...
        foreach ($data as $key=>$item) {
			$info[(int)$key] = HOST . '/index/mapstatic?id='.$item['fname'].'&ver='.$item['ver'];
		}
		return $info;
	}
	...
	public static function getStrengThenClass()
	{
		$info = array();
		$data = Hapyfish2_Alchemy_Cache_Basic::getMercenaryStrengthenList();
		...
		foreach ($data as $v) {
			$info[] = array(
					...
					'type' => (int)$v['type'],
					'level' => (int)$v['level'],
					'strMax' => (int)$v['max_str'],
					'dexMax' => (int)$v['max_dex'],
					'magMax' => (int)$v['max_mag'],
					'phyMax' => (int)$v['max_phy']
			);
		}
	...
	public static function getHireCountMaxList()
	{
		$info = array(0);
		$data = Hapyfish2_Alchemy_Cache_Basic::getTavernLevelList();
	 ...
	 */
	public static function getQQYellowVipDailyGift()
	{
		$list = Hapyfish2_Alchemy_Cache_BasicExt::getQQYellowVipGiftList();
		$data = array();
	
		foreach ($list as $v) {
			$data[] = json_decode($v['awards'], true);
		}
