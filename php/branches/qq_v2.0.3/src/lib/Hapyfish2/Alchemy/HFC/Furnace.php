...
class Hapyfish2_Alchemy_HFC_Furnace
{
    ...
    public static function addOne($uid, $furnace)
    {
    	...
    	try {
    		...
    		if ($id > 0) {
	            ...
	            //添加图鉴
	            Hapyfish2_Alchemy_Bll_Illustrations::addUserIllustrations($uid, $furnace['furnace_id']);
    		}
    	} catch (Exception $e) {
    ...
    public static function getOneCanUseByCid($uid, $cid)
    {
    	$data = self::getOnRoom($uid);
    	if (empty($data)) {
    		return array();
    	}
    	
    	$furnaces = array();
    	foreach ($data['furnaces'] as $item) {
    		if ( $item['furnace_id'] == $cid && $item['cid'] == 0 ) {
    			$furnaces = $item;
    		}
    	}
     ...
     * 炼金炉 当前格子数
     * @param int $uid
     * @param int $cid,炼金炉cid
     * @return int
     */
    public static function getFurnaceCurNum($uid, $cid)
    {
    	$data = self::getAll($uid);
    	if (empty($data)) {
    		return 0;
    ...
    public static function updateOne($uid, $id, $furnace, $savedb = true)
    {
        ...
        $data = array(
            ...
            $furnace['start_time'], $furnace['need_time'], $furnace['cur_probability'], $furnace['num'], $furnace['status'], $furnace['idx']
        );
    ...
    public static function getCurMixs($uid)
    {
    	...
    	foreach ( $furnacesList as $v ) {
    		if ( $v['cid'] > 0 && $v['status'] == 1 ) {
    			...
    			$fid = $v['id'];
    			$curMixs[] = array('id' => $fid,
    							   'furnaceCid' => $v['furnace_id'],
    							   'cid' => $v['cid'],
