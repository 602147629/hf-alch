...
$list['line_0'] = array(
    ...
    'name'	    => 'line'
		...
		$list['alchemy_helltower_config'] = array(
		    'name'	    => '地狱塔配置表',
		    'tbid'		=> 'alchemy_helltower_config',
			'key'		=> 'id',
			'column'	=> array('id'=>'层数', 'awards'=>'奖励','type'=>'地狱or深渊', 'monster'=>'显示怪物','monster_stations'=>'怪物站位')
		);
		$list['alchemy_helltower_shop'] = array(
		    'name'	    => '地狱塔商店',
		    'tbid'		=> 'alchemy_helltower_shop',
			'key'		=> 'cid',
			'column'	=> array('cid'=>'道具id', 'price'=>'价格','vip_price'=>'vip价格', 'needWater'=>'水牢解锁','needAbyss'=>'深渊解锁','order'=>'排序')
		);
...
$list['line_1'] = array(
    ...
    'name'	    => 'line'
		...
		$cKeys = array('map_id','name','cost_num','order','group','entrance_id','reachable','link_list');
		$cVals = array('副本id','名称','需要道具数','排序id','地图组名称','入口场景id','能否到达,1:能,0:否', '传送门引导列表-link_list');
	   ...
	   /*  $list['alchemy_mercenary_model_old'] = array(
	    ...
	    $list['alchemy_mercenary_name'] = array(
			...
			'column'	=> array('id'=>'id', 'name'=>'name', 'content'=>'content', 'sex'=>'sex',
								 ...
								 'job'=>'job', 'hire_type'=>'hire_type', 'hire_level'=>'hire_level', 'rp'=>'佣兵星级')
		);
	    ...
	    $list['alchemy_mercenary_strengthen'] = array(
	    		...
	    		'column'	=> array('id'=>'id', 'job'=>'job', 'type'=>'type', 'level'=>'level',
	    				...
	    				'k_str'=>'k_str', 'k_dex'=>'k_dex', 'k_mag'=>'k_mag', 'k_phy'=>'k_phy', 'm'=>'m', 'n'=>'n')
	    );
 ...
 */
	    ...
	    $cKeys = array('rp','type','add_score','need_coin');
	    $cVals = array('星级（1-10）','刷新类型（0-7）','增加积分数','需要金币数');
	    $list['alchemy_hire_rp'] = array(
	    		'name'	    => '酒馆刷新-星级、类型信息表',
	    		'tbid'		=> 'alchemy_hire_rp',
	    		'key'		=> 'rp',
	    		'column'	=> array_combine($cKeys, $cVals)
	    );
	    
	    $cKeys = array('rp','max_score','need_score');
	    $cVals = array('星级（1-10）','最大积分数（不包含临界值）','雇佣需要积分数');
	    $list['alchemy_hire_rp_score'] = array(
	    		'name'	    => '酒馆刷新-星级积分信息表',
	    		'tbid'		=> 'alchemy_hire_rp_score',
	    		'key'		=> 'rp',
	    		'column'	=> array_combine($cKeys, $cVals)
	    );
...
$list['line_7'] = array(
    ...
    'name'	    => 'line'
		...
		$list['alchemy_level_build'] = array(
				'name'	    => '建筑升级TIPS表',
				'tbid'		=> 'alchemy_level_build',
				'key'		=> 'id',
				'column'	=> array('id'=>'id', 'name'=>'name', 'max_level' => 'max_level', 'level_tips'=>'level_tips', 'lock_name'=>'lock_name', 'order'=>'排序')
		);
	    ...
	    $list['alchemy_level_tavern'] = array(
			...
			'column'	=> array('id'=>'id', 'level'=>'level', 'content'=>'content', 'need_level'=>'need_level',
								 ...
								 'npc_class'=>'npc_class', 'npc_chat'=>'npc_chat', 'refresh_price'=>'refresh_price', 'need_time'=>'需要时间(秒)', 'cd_time' => 'cd_time', 'max_score' => '最大人气')
		);
...
$list['line_9'] = array(
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_arena_prize'] = array(
	    		'name'	    => '竞技场排名奖励',
	    		'tbid'		=> 'alchemy_arena_prize',
	    		'key'		=> 'rank',
	    		'column'	=> array('rank'=>'排名', 'coin'=>'奖励金币', 'feats'=>'奖励神勇点')
	    );
...
$list['line_10'] = array(
	    ...
	    $list['alchemy_qq_yellowvip_dailygift'] = array(
	    		'name'	    => '黄钻等级每日礼包',
	    		'tbid'		=> 'alchemy_qq_yellowvip_dailygift',
	    		'key'		=> 'level',
	    		'column'	=> array('level'=>'level', 'awards'=>'awards')
	    );
...
$list['line_12'] = array(
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_stat_user_action'] = array(
		 ...
		 $list['alchemy_event_change'] = array(
            'name' => '聚神点金',
            'tbid' => 'alchemy_event_change',
            'key' => 'user_level',
            'column' => array('user_level' => 'user_level', 'gold_change' => 'gold_change', 'brova_change' => 'brova_change','gold_diamondcost'=>"gold_diamondcost",'brova_diamondcost'=>'brova_diamondcost')
        );
        $list['alchemy_special_shop'] = array(
            'name' => '日常优惠礼包',
            'tbid' => 'alchemy_special_shop',
            'key' => 'id',
            'column' => array('id' => 'id', 'type' => '礼包类型(1强化2材料3图纸4血魔5经验)', 'can_buy' => '可购数量','buyLimit'=>'可购数量','total'=>'总数','buyMore'=>'是否可超上限')
        );
        $list['alchemy_special_shop_item'] = array(
            'name' => '日常优惠礼包内容',
            'tbid' => 'alchemy_special_shop_item',
            'key' => 'id',
            'column' => array('id' => '对应礼包id', 'level' => '战斗等级', 'awards' => '礼包内容','price'=>"原价",'newPrice'=>'优惠价')
        );
