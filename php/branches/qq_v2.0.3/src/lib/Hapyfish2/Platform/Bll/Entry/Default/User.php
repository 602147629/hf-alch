...
class Hapyfish2_Platform_Bll_Entry_Default_User
{
    ...
    public static function getUser($uid)
    {
        ...
        $data = $hc->get($key);
        if (! $data) {
            $data = Hapyfish2_Platform_Cache_Entry_Default_User::getUser($uid);
            if ($data) {
                $hc->set($key, $data);
            }
        }
    ...
    public static function getMultiUser($fids)
    {
        $info = array();
        foreach ($fids as $fid) {
            $user = Hapyfish2_Platform_Cache_Entry_Default_User::getUser($fid);
            if ($user && $user['puid']) {
                $info[] = array(
                    'uid' => $user['uid'], 
                    'name' => $user['name'], 
    ...
    {
        //get
        $old = Hapyfish2_Platform_Cache_Entry_Default_User::getUser($uid);
        if ($old) {
        ...
        $res = Hapyfish2_Platform_Cache_Entry_Default_User::updateUser($uid, $user, $savedb);
        if ($res) {
            $hc = Hapyfish2_Cache_HighCache::getInstance();
            $key = 'p:u:' . $uid;
            $data = array(
    ...
    {
        $res = Hapyfish2_Platform_Cache_Entry_Default_User::addUser($user);
        if ($res) {
            $hc = Hapyfish2_Cache_HighCache::getInstance();
            $uid = $user['uid'];
            $key = 'p:u:' . $uid;
