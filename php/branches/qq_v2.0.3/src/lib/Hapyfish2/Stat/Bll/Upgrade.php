...
class Hapyfish2_Stat_Bll_Upgrade
{
	 ...
	 */
    public static function calUpgrade($dt, $dir)
    {
    	$data = null;
	...
	private static function _getStat($dt, $dir, $file)
	{
    	...
    	$data = array();
        $fileName = $dir .'/'.$file.'/'.$dt . '/all-'.$file.'-' . $dt . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
                info_log($fileName . ' not exists!', 'stat_upgrade');
                return false;
            }
            $content = file_get_contents($fileName);
            if (!$content) {
                info_log($fileName . ' has no content!', 'stat_upgrade');
                return false;
            }
            $lines = explode("\n", $content);
            $params351 = array();
