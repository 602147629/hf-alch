...
require_once 'OpenApi/SinaWeibo/Weiyouxi.php';

class OpenApi_SinaWeibo_Client
{
    protected $appKey;
    protected $appSecret;
    protected $appId;
    ...
    public function __construct($appKey, $appSecret)
    {
        ...
        $this->rest = new OpenApi_SinaWeibo_Weiyouxi($appKey, $appSecret);
    }
...
//info_log(Zend_Json::encode($data), 'sina_user');
                ...
                $user['vuid'] = $data['verified_type'];
                $user['identity'] = $data['identity_status'];
...
//info_log(Zend_Json::encode($data), 'sina_app_friend');
    ...
    public function setRank($rankId, $value)
    {
        try {
        	...
        	info_log(json_encode($data),'rank');
            if(isset($data)) {
                if (isset($data['error_code'])) {
    ...
    /* 支付API */
    //获得支付token
    public function getPayTokenTest($orderId, $amount, $desc, $sign)
    {
    	try {
    		$aryParam = array('order_id'=>$orderId, 'amount'=>$amount, 'desc'=>$desc, 'sign'=>$sign);
    		$data = $this->rest->post('test/pay/get_token', $aryParam);
    		if(isset($data)) {
    			if (isset($data['error_code'])) {
    				info_log('[SinaWeibo_Client::getPayToken]: ' . $data['error_code'].$data['error'], 'wb_api_call_err');
    				return null;
    ...
    //查询订单状态
    public function getPayStatusTest($orderId, $puid, $appId, $sign)
    {
    	try {
    		$aryParam = array('order_id'=>$orderId, 'user_id'=>$puid, 'app_id'=>$appId, 'sign'=>$sign);
    		$data = $this->rest->get('test/pay/order_status', $aryParam);
    		if(isset($data)) {
    			if (isset($data['error_code'])) {
    				info_log('[SinaWeibo_Client::getPayStatus]: ' . $data['error_code'].$data['error'], 'wb_api_call_err');
    				return null;
    ...
    {
    	 try {
        	$data = $this->rest->get('user/online_time');
            if(isset($data)) {
                if (isset($data['error_code'])) {
                    info_log('[SinaWeibo_Client::getPayStatus]: ' . $data['error_code'].$data['error'], 'wb_api_call_err');
                    return null;
            ...
            info_log('[SinaWeibo_Client::online_time]: ' . $e->getMessage(), 'wb_api_call_err');
        }
