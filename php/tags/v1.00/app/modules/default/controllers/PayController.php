...
/**
 * Alchemy pay controller
 *
 * @copyright  Copyright (c) 2010 HapyFish
 * @create      2012/05    zx
 */
class PayController extends Hapyfish2_Controller_Action_Api
{
    protected $uid;
	...
	public function topAction()
	{
		...
		$user['gold'] = Hapyfish2_Alchemy_HFC_User::getUserGold($uid);
	...
	public function orderlistAction()
	{
		...
		$listOrder = Hapyfish2_Alchemy_Bll_Payment::getOrderList($uid);
    ...
    public function logAction()
    {
		...
		$user = Hapyfish2_Platform_Bll_User::getUser($uid);
		$user['face'] = $user['figureurl'];
		$user['gold'] = Hapyfish2_Alchemy_HFC_User::getUserGem($uid);


		$dalPayLog = Hapyfish2_Alchemy_Dal_PayLog::getDefaultInstance();
    	$logs = $dalPayLog->listPayLog($uid, 50);
    	if (!$logs) {
    		$count = 0;
    		$logs = '[]';
    	} else {
 ...
 //debug_log(json_encode($_POST));
        ...
        $token = $token['token'];
        $order = Hapyfish2_Alchemy_Bll_Payment::createOrder($uid, $orderid, $amount, $gold, $time, $token);
        if ($order) {
