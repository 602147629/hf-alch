...
class Hapyfish2_Admin_Bll_Basic
{
    ...
    public static function importMapCopyDataFromFile($tbName, $fileName, &$aryFailed)
    {
        ...
        try {
            ...
            $updField = '';
            if ($tbName == 'monster') {
                //id,sceneId,cid,x,z,fiddleRangeX,fiddleRangeZ,monsterper,detail,collision,goHome -> id,cid,x,z,fr_x,fr_z,per,detail,end,fightBg
                $cntFields = 12;
            }
            else if ($tbName == 'mine') {
...
$list['line_0'] = array(
    ...
    'name'	    => 'line'
		...
		$cKeys = array('id','tid','level','skill_id','element','job','need_count','need_lev');
		$cVals = array('id','组id','援攻等级','技能id','需要属性','需要职业','需要好友数','需要等级 ');
...
$list['line_1'] = array(
    ...
    'name'	    => 'line'
		...
		$cKeys = array('cid','tid','job','name','content','avatar_class_name','class_name','face_class_name','s_face_class_name','element','level','hp','mp','phy_att','phy_def','mag_att','mag_def','agility','crit','dodge','size_x','size_z','collision_range','is_boss','weapon','skill','award_conditions','first_award_conditions','award_exp','talk');
		$cVals = array('cid','组id','职业','name','content','avatar_class_name','class_name','face_class_name','s_face_class_name','1风2火3水','level','hp','mp','phy_att','phy_def','mag_att','mag_def','agility','crit','dodge','size_x','size_z','碰撞范围','是否boss','武器','技能','击杀奖励','首杀奖励','基础Exp','开战对白');
		...
		$cKeys = array('cid','enter_scene','name','content','class_name','x','y','need_sp','links', 'role_level');
		$cVals = array('cid','enter_scene','name','content','class_name','x','y','need_sp','links', 'role_level');
...
$list['line_99'] = array(
    ...
    'name'	    => 'line'
		...
		$cKeys = array('id','desp','condition_type','cid','icon_cid','num','classname_type');
		$cVals = array('id','描述','任务逻辑类型id','类id','图片id','数量','前端区分表现图片用type');
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_goods'] = array(
			...
			'column'	=> array('cid'=>'cid', 'type'=>'type', 'func_id'=>'物品作用cid', 'name'=>'name', 'tips'=>'tips', 'content'=>'content',
								 ...
								 'buy_gem'=>'buy_gem', 'sale_coin'=>'售系统价格', 'worth'=>'售顾客价格', 'lose'=>'是否消耗(1消耗)', 'label'=>'label', 'buy_level'=>'购买需要等级', 'sort'=>'排序')
		);
	    ...
	    $list['alchemy_equipment'] = array(
			...
			'column'	=> array('cid'=>'cid', 'type'=>'type', 'level'=>'需要等级', 'jobs'=>'职业列表', 'pa'=>'物攻',
								 'pd' => '物防', 'ma'=>'魔攻', 'md'=>'魔防', 'speed'=>'速度', 'hp'=>'hp加成', 'mp'=>'mp加成', 'cri' => '爆击', 'dod'=>'闪避',
	    						 'name'=>'name', 'content'=>'content', 'class_name'=>'class_name', 'is_new'=>'is_new', 'can_buy'=>'can_buy',
	    						 'buy_coin'=>'buy_coin', 'buy_gem'=>'buy_gem', 'sale_coin'=>'售系统价格', 'worth'=>'售顾客价格', 'durability'=>'耐久',
	    						 'can_repair' => '是否可修', 'repair_level' => '修理等级', 'buy_level'=>'购买需要等级', 'sort'=>'排序')
		);
	    ...
	    $list['alchemy_mix'] = array(
			...
			'column'	=> array('cid'=>'cid', 'furnace_cid'=>'需要工作台cid', 'item_cid'=>'合成物品cid', 'fail_item'=>'失败获得材料',
	    						 'name'=>'name', 'coin'=>'需要金币', 'gem'=>'需要宝石', 'needs'=>'需要材料', 'sp'=>'需要sp',
...
$list['line_4'] = array(
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_order'] = array(
		    ...
		    'tbid'		=> 'alchemy_order',
			'key'		=> 'cid',
			'column'	=> array('cid'=>'cid', 'needs'=>'nedds', 'out_time'=>'out_time', 'need_level'=>'need_level', 'coin'=>'coin',
...
$list['line_5'] = array(
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_mercenary_grow_class'] = array(
			...
			'key'		=> 'rp',
			'column'	=> array('rp'=>'rp', 'element'=>'职业 1-战士，2-弓手，3-法师', 'hp'=>'hp', 'mp'=>'mp', 'phy_att'=>'phy_att', 'phy_def'=>'phy_def', 'mag_att'=>'mag_att', 'mag_def'=>'mag_def', 'agility'=>'agility')
		);
	    ...
	    $list['alchemy_mercenary_grow'] = array(
			...
			'column'	=> array('id'=>'id', 'rp'=>'rp', 'job'=>'job', 'hp'=>'hp', 'mp'=>'mp', 'phy_att'=>'phy_att', 'phy_def'=>'phy_def',
								 'mag_att'=>'mag_att', 'mag_def'=>'mag_def', 'agility'=>'agility', 'crit'=>'crit', 'dodge'=>'dodge',
								 'coin'=>'coin', 'skill'=>'skill', 'hire_type'=>'hire_type')
		);
	    ...
	    $list['alchemy_mercenary_strengthen'] = array(
			...
			'column'	=> array('id'=>'id', 'job'=>'job', 'type'=>'type', 'level'=>'level', 
								 ...
								 'min_agility'=>'min_agility', 'min_crit'=>'min_crit', 'min_dodge'=>'min_dodge',
								 'max_phy_att'=>'max_phy_att', 'max_phy_def'=>'max_phy_def', 'max_mag_att'=>'max_mag_att', 'max_mag_def'=>'max_mag_def', 
								 'max_agility'=>'max_agility', 'max_crit'=>'max_crit', 'max_dodge'=>'max_dodge')
		);
...
$list['line_7'] = array(
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_level_home'] = array(
			...
			'key'		=> 'level',
			'column'	=> array('level'=>'level', 'order_count'=>'order_count', 'tile_x_length'=>'tile_x_length', 'tile_z_length'=>'tile_z_length',
								 'content'=>'content', 'need_level'=>'need_level', 'need_items'=>'need_items', 'need_coin'=>'need_coin', 'need_prestige'=>'need_prestige',
								 'npc_class'=>'npc_class', 'npc_chat'=>'npc_chat')
		);
	    ...
	    $list['alchemy_level_tavern'] = array(
			...
			'column'	=> array('id'=>'id', 'level'=>'level', 'content'=>'content', 'need_level'=>'need_level',
								 ...
								 'npc_class'=>'npc_class', 'npc_chat'=>'npc_chat', 'refresh_price'=>'refresh_price')
		);
    ...
    'name'	    => 'line'
	    ...
	    $list['alchemy_init_role'] = array(
			...
			'column'	=> array('id'=>'id', 'rp'=>'rp', 'gid'=>'gid', 'job'=>'job', 'sex'=>'sex', 'avatar'=>'avatar',
								 ...
								 'mag_att'=>'mag_att', 'mag_def'=>'mag_def', 'agility'=>'agility', 'crit'=>'crit', 'dodge'=>'dodge',
								 'name'=>'临时英雄名字', 'element'=>'临时英雄属性（风火水）', 'weapon'=>'临时英雄装备', 'task_id'=>'临时英雄对应任务id')
		);
