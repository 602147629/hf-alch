...
class Hapyfish2_Alchemy_Bll_Fight
{
	...
	public static function regFight($uid, $id)
	{
	    ...
	    foreach ($enemySide as $key => $data) {
	        ...
	        //添加遇到怪物记录，并判断是否有首杀奖励
            $addMonster = Hapyfish2_Alchemy_HFC_Monster::addMonster($uid, $data['cid']);
            if ( $addMonster ) {
...
//info_log($pos.':'.$v[1].':'.($v[0]-$data['level']).':'.$awardExp, 'exp');
                    ...
                    if ($levelUp) {
                        ...
                        $expVo[] = array($pos, $awardExp, (int)$userMerc['level'] + 1, (int)$uexp, (int)$nextLevelExp);
                    }
                    else {
                        $expVo[] = array($pos, $awardExp, 0);
            ...
            //失败 逃跑 其他 结算
                ...
                foreach ($enemySide as $data) {
                    ...
                    if ($data['is_boss']) {
                        //统计log
                        $log->report('222', array($uid, $mapId, $data['cid'], $rst['result']));
                    }
                    //怪物组别
                    else {
                        //统计log
                        $log->report('221', array($uid, $mapId, $data['cid'], $rst['result']));
                    }
                }
            }//end if 失败 逃跑 其他
            ...
            if (!isset($resultVo['rolesChange'])) {
                ...
                //$rolesChange = Hapyfish2_Alchemy_Bll_FightCorps::genHomeRolesVo($uid, $changedMerc);
                //Hapyfish2_Alchemy_Bll_UserResult::addField($uid, 'rolesChange', $rolesChange);
            }
	    ...
	    //新手引导战斗完成后触发剧情
	    if ( $id == 100 ) {
	        Hapyfish2_Alchemy_Bll_Story::startStory($uid, 31);
	    }
	 ...
	 * @param int $baseExp
	 * @param int $levDiff
	 * @param int $coefNum1 加成值%
	 * @param int $coefNum2 衰减值%
	 * @param int $coefNum3 上限值%
	 * @param int $coefNum4 下限值%
	 * @return integer
	 */
	private static function _calcExpByLevDiff($baseExp, $levDiff, $coefNum1 = 5, $coefNum2 = 5, $coefNum3 = 1, $coefNum4 = 300)
	{
        if (!$baseExp) {
            return 0;
        }
...
//echo json_encode($friendJobLevStat);exit;
        ...
        foreach ($basAssist as $assData) {
            ...
            if (1 == $assData['level']) {
                $aryAssist[(int)$assData['tid']] = array('avatar'=>'', 'name'=>'', 'lev'=>0,
                										 'skillId'=>(int)$assData['skill_id'],
                										 'next'=>array((int)$assData['need_count'], (int)$assData['need_lev'], (int)$assData['job'], (int)$assData['element']));
            }
