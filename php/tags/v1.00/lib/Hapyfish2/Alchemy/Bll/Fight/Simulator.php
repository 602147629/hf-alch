...
class Hapyfish2_Alchemy_Bll_Fight_Simulator
{
    ...
    const EFFECT_TYPE_ADD_CHANGE_PHYDEF_STATUS = 7;//增加影响物防状态
    ...
    protected function _phyAtt($actorPos, $targetPos)
	{
        ...
        //是否闪避
        if ($this->_isDodge($actor['dodge'], $target['dodge'])) {
            $this->_logStep($logAttMeth.':'.$actorPos.'->'.$targetPos . ' MISS');
            return;
        }

        //是否暴击
        $isCrit = $this->_isCrit($actor['crit'], $target['crit']);
        ...
        if ($effectList) {
            foreach ($effectList as $key=>$data) {
                ...
                if ($data['duration'] >= 0) {
                    ...
                    else if ($type == self::EFFECT_TYPE_ADD_CHANGE_PHYDEF_STATUS) {
                        $prop = 'phy_def';
                    }
	        ...
	        if (isset($this->_basSkillList[$cid])) {
	            ...
	            if (isset($skillInfo['resist']) && $skillInfo['resist'] > 0) {
    	            ...
    	            if ($skillInfo['dodge_accept']) {
                        //是否闪避
                        if ($this->_isDodge($actor['dodge'], $target['dodge']*($skillInfo['dodge_accept']/100))) {
	            ...
	            if ($skillInfo['crit_accept']) {
                    //是否暴击
                    $isCrit = $this->_isCrit($actor['crit']*($skillInfo['crit_accept']/100), $target['crit']);
                }
        ...
        //消除所有负面状态
            ...
            switch ($effType) {
                ...
                case self::EFFECT_TYPE_REMOVE_CHANGE_PHYDEF_STATUS ://消除影响物防状态
                    $prepEff[] = self::EFFECT_TYPE_ADD_CHANGE_PHYDEF_STATUS;
                    break;
	...
	protected function _extStatusDeal($pos)
	{
	    ...
	    foreach ($effectList as $key=>&$data) {
	        ...
	        if ($data['duration'] < 0) {
    	        ...
    	        $type = $data['type'];
    	        //hp change
                if ($type == self::EFFECT_TYPE_ADD_CHANGE_HP_STATUS) {
                ...
                //无法行动
                        ...
                        || $type == self::EFFECT_TYPE_ADD_SLEEP_STATUS) {
                    $skip = true;
    ...
    protected function _calcHurtVal($attVal, $defVal, $adjust)
	{
	    if ($adjust) {
	        //相克性修正率
	        $hurtVal = ceil( ($attVal*($attVal/($attVal+$defVal)*self::BASE_COEFFICIENT)*(1+$adjust/100)) );
	        $this->_logStep('hurtAdj:'.($adjust/100));
	    }
	    else {
	        $hurtVal = ceil( ($attVal*($attVal/($attVal+$defVal)*self::BASE_COEFFICIENT)) );
	    }
        return $hurtVal;
	}
    ...
    protected function _isDodge($actDodge, $tarDodge)
	{
        ...
        //$dodgeRate = ($tarDodge*($tarDodge/($tarDodge+$actDodge)*self::BASE_COEFFICIENT) * 10);
        $dodgeRate = ($tarDodge*($tarDodge/($tarDodge+$actDodge)*self::BASE_COEFFICIENT));
        $randomDig = $this->_rndElement[$this->_rndIterate];
        $this->_rndNumUsed();
	...
	protected function _isCrit($actCrit, $tarCrit)
	{
        ...
        $critRate = ($actCrit*($actCrit/($actCrit+$tarCrit)*self::BASE_COEFFICIENT));
	    $randomDig = $this->_rndElement[$this->_rndIterate];
        $this->_rndNumUsed();
