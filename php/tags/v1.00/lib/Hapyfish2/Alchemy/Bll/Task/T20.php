 ...
 * type = 20
 */
class Hapyfish2_Alchemy_Bll_Task_T20
{
	public function trigger($uid, $openTask, $taskInfo, $taskConditionInfo, $data)
	{
		...
		foreach ($data as $cid=>$num) {
		    ...
		    if ($cid == $taskConditionInfo['cid']) {
        		...
        		$canComplete = false;
        		$curNum = Hapyfish2_Alchemy_Bll_BagItemDict::getItemCntByCid($uid, $cid);
        		if ($curNum >= $taskConditionInfo['num']) {
                    $openTask['data'][$taskId][$conditionId] = (int)$taskConditionInfo['num'];
                    $canComplete = true;
        		}
        		else {
                    $openTask['data'][$taskId][$conditionId] = $curNum;
        		}
