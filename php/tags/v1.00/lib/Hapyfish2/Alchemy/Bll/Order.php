...
class Hapyfish2_Alchemy_Bll_Order
{
	...
	public static function completeOrder($uid, $id, $isFailed, $wid)
	{
		...
		foreach	( $needs as	$need )	{
			...
			if ( $needType == 1	) {
				...
				$userNewWeapon = Hapyfish2_Alchemy_HFC_Weapon::getOne($uid, $wid);
				if ( !$userNewWeapon || $userNewWeapon['durability'] < 1000) {
					return -204;
				}
				$needWeapon[] =	array('wid'	=> $wid, 'count' =>	$needCount, 'cid'=>$userNewWeapon['cid']);
			}
		}
		...
		if ( !empty($needWeapon) ) {
			foreach	( $needWeapon as $weapon ) {
				$ok = Hapyfish2_Alchemy_HFC_Weapon::delWeapon($uid, $wid);
				if ( $ok ) {
					$removeItems[] = array($weapon['cid'], 1, $wid);
				}
			}
		}
	...
	{
		//随机获得订单资质等级
		$awardType = self::_getRandAwardType($awardType);
	...
	public static function _getRandAwardType($awardType)
	{
		//-1:自家普通订单,60/30/9/1
		if ( $awardType == -1 ) {
			...
			else if ( $randAward <= 90 ) {
				$awardType = 2;
			}
			else if ( $randAward <= 99 ) {
				$awardType = 3;
			}
			else {
				$awardType = 4;
		...
		//-2:宝石刷新订单,50/25/15/10
		else if ( $awardType == -2 ) {
			$randAward = rand(1,100);
			if ( $randAward <= 50) {
				$awardType = 1;
			}
			else if ( $randAward <= 75 ) {
				$awardType = 2;
			}
			else if ( $randAward <= 90 ) {
				$awardType = 3;
