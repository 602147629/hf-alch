...
class Hapyfish2_Alchemy_Bll_Vip
{
	...
	const VIP_CONINUE = 600; //vip特权延续时间
	const DAY_DEBUFF = 10;
	const LIMIT = 10;
	private static $_skipFight = array(20,40,80,150,150);
	protected $_levelArr = array(0,10,11,12,13,15); //卡片成长
	...
	protected $_levelExp = array(600,1500,3000,6000); //vip经验 
	...
	{
		$awardAry = array('1' => '[[715,2],[4415,1],[2715,1],[5015,3]]',
						  '2' => '[[715,2],[4415,1],[2715,1],[5015,3]]',
						  '3' => '[[715,3],[4415,2],[2715,2],[5015,4],[915,1]]',
						  '4' => '[[715,3],[4415,2],[2715,2],[5015,4],[915,1]]',
						  '5' => '[[715,4],[4415,3],[2715,3],[5015,5],[915,1]]');
		return $awardAry;
	}
	...
	public function getVipSkip($uid)
	{
		...
		if($info && $this->_vipStatus != true && $userSkip['max'] > 5){
			$userSkip['max'] = 5;
	...
	public  function getVipStatic()
	{
		...
		return array('vipTable'=>array('skipBattle'=>$skip, 'fixEquip'=>$repair),'vipGrowUp'=>$levelArr);
	}
	...
	public function userVipDayCard($uid)
	{
		...
		if($info['type'] == 0){
			$this->_vipInfo['uid'] = $uid;
			$this->_vipInfo['starttime'] = $time;
			$this->_vipInfo['endtime'] = $time + 86400 * 1;
			$this->_vipInfo['level'] = 1;
			$this->_vipInfo['growup'] = 0;
