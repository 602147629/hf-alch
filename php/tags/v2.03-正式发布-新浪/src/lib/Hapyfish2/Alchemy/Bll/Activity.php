...
class Hapyfish2_Alchemy_Bll_Activity
{
	...
	public static function getTaskVo($uid)
	{
        ...
        $activityVo['currentLiveness'] = $userActivity['activity'];
        $activityVo['maxLiveness'] = self::getMaxActivity();
        $activityVo['list'] = self::getComplete($userActivity['activity'],json_decode($userActivity['step']));
	...
	public static function getActivityAward($uid, $type)
	{
		...
		if($userAc['activity'] < $info['activity']){
			return -704;
		}
		...
		$userAc = Hapyfish2_Alchemy_Cache_Activity::get($uid);
		$list = self::getComplete($userAc['activity'],json_decode($userAc['step']));
