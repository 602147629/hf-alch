...
class Hapyfish2_Alchemy_HFC_MapCopy
{
	 ...
	 * @param int $uid
	 */
	public static function getOpenTransport($uid)
    {
    	$key = 'a:u:opentransport:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getHFC($uid);
		$data = $cache->get($key);
		if($data === false) {
			try {
    ...
    public static function updateOpenTransport($uid, $transport, $savedb = true)
    {
        $key = 'a:u:opentransport:' . $uid;
        $cache = Hapyfish2_Cache_Factory::getHFC($uid);
        ...
        if ($savedb) {
            ...
            if ($ok) {
                ...
                try {
                    ...
                    info_log('[Hapyfish2_Alchemy_HFC_MapCopy::updateOpenTransport]' . $e->getMessage(), 'db.err');
                }
            }
        } else {
            $ok = $cache->update($key, $data);
     ...
     * @param int $pid,门id
     */
    public static function addOpenTransport($uid, $pid)
    {
    	$list = self::getOpenTransport($uid);
    	if ( !in_array($pid, $list) ) {
    		$list[] = $pid;
    		return self::updateOpenTransport($uid, $list);
    	}
    	return;
    }
    
    public static function getUserRefreshMap($uid)
    {
    	$key = 'a:u:refreshMap:'.$uid;
    	$cache = Hapyfish2_Cache_Factory::getHFC($uid);
    	$data = $cache->get($key);
    	$date = date('Ymd');
    ...
    public static function updateUserRefreshMap($uid, $data)
    {
    	$key = 'a:u:refreshMap:'.$uid;
    	$cache = Hapyfish2_Cache_Factory::getHFC($uid);
