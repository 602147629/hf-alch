...
class EditstoryController extends Zend_Controller_Action
{
    ...
    {
        $uid = (int)$this->_request->getParam('uid', null);
        if ( !$uid ) {
        	$uid = 10019;
        	$this->uid = 10019;
        }
    ...
    public function beginfightAction()
    {
    	...
    	$status = Hapyfish2_Alchemy_Bll_Test::regFight($uid, $id);
    	if ($status < 0) {
    		$this->echoError($status);
    	}
    	$this->flush();
    ...
    public function testdbAction()
    {
    	...
    	$status = Hapyfish2_Alchemy_Bll_Test::testDb($uid, $id);
    	if ($status < 0) {
    		$this->echoError($status);
    	}
    	$this->flush();
    ...
    public function testcacheAction()
    {
    	...
    	$status = Hapyfish2_Alchemy_Bll_Test::testCache($uid, $id);
    	if ($status < 0) {
    		$this->echoError($status);
    	}
    	$this->flush();
