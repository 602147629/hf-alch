...
class Hapyfish2_Alchemy_Dal_Fight
{
    ...
    public function insUpd($uid, $info)
    {
        ...
        $sql = "INSERT INTO $tbname (uid, fid, `type`, enemy_id, `status`, rnd_element, home_side, enemy_side, content, create_time, new_monster)
        		VALUES (:uid, :fid, :type, :enemy_id, :status, :rnd_element, :home_side, :enemy_side, :content, :create_time, :new_monster)
        ...
        return $wdb->query($sql, array('uid'=>$uid, 'fid'=>$info['fid'], 'type'=>$info['type'], 'enemy_id'=>$info['enemy_id'], 'status'=>$info['status'],
        							   ...
        							   'content'=>$info['content'], 'create_time'=>$info['create_time'], 'new_monster'=>$info['new_monster']));
    }
