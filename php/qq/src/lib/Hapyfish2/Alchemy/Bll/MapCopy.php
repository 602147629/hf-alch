			...
			if($access < 10){
				if(!in_array($fid, $userAward['list'])){
					...
					if($userSp['sp'] < $maxSp){
						...
						$ok = Hapyfish2_Alchemy_HFC_User::incUserCoin($uid, $addCoin);
						$userAward['list'][] = $fid;
						Hapyfish2_Alchemy_Cache_User::updateAccess($uid, $userAward);
						$showAward= array(
							'name'=>'访问好友可以获得奖励：',
							'tips'=>'每天访问好友奖励可以获得10次',
							'hideLogo'=>1,
			...
			if ($v['type'] == 1) {
    			...
    			switch ($type) {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                    	if($fromType == 3 && $cid == 10961){
	                    	for ($i=0; $i< $num; $i++) {
	    						Hapyfish2_Alchemy_Bll_EventGift::addUserWeapon($uid, $cid, 2);
	    					}
                    	}else{
