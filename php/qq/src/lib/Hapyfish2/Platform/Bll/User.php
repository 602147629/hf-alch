...
class Hapyfish2_Platform_Bll_User
{
    ...
    {
    	$online = array();
    	$userIdentity = Hapyfish2_Platform_Cache_User::getIdentity($uid);
    	if($userIdentity == 2){
    	...
    	Hapyfish2_Platform_Cache_User::updateOnlineTime($uid, $data);
    	return array('sinaAntiState'=>$userIdentity, 'antiTime'=>$online['online_time']);
    }
    
    public static function getIdentityExp($uid,$exp)
    {
    	$onlineTime = Hapyfish2_Platform_Cache_User::getOnlineTime($uid);
