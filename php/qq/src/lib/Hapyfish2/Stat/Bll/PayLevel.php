...
class Hapyfish2_Stat_Bll_PayLevel
{
    ...
    public static function calcGemLevel($dt, $dir)
    {
    	...
    	$num = 0;
    	$pnum = 0;
        $fileName = $dir .'/payLevel/'. $dt . '/all-GemLevel-' . $dt . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
        ...
        $list = array('date'=>$dt, 'level'=>json_encode($data),'type'=>json_encode($data1));
        $dal = Hapyfish2_Stat_Dal_PayLevel::getDefaultInstance();
        $dal->insert('day_GemLevel',$list);
    }
    ...
    public static function calcBuyLevel($dt, $dir)
    {
    	...
    	$num = 0;
    	$pnum = 0;
        $fileName = $dir .'/payLevel/'. $dt . '/all-buyLevel-' . $dt . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
                info_log($fileName . ' not exists!', 'GemLevel');
