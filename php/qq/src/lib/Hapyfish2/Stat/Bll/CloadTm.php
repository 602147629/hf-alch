...
class Hapyfish2_Stat_Bll_CloadTm
{
    ...
    public static function calcDayData($dt, $dir)
    {
        ...
        try {
            ...
            $cntPie_tm3_4_1 = $cntPie_tm3_4_3 = $cntPie_tm3_4_5 = $cntPie_tm3_4_10 = $cntPie_tm3_4_15 = $cntPie_tm3_4_30 = $cntPie_tm3_4_60 = $cntPie_tm3_4_oth = 0;
            $cntPie_tm3_4_1_new = $cntPie_tm3_4_3_new = $cntPie_tm3_4_5_new = $cntPie_tm3_4_10_new = $cntPie_tm3_4_15_new = $cntPie_tm3_4_30_new = $cntPie_tm3_4_60_new = $cntPie_tm3_4_oth_new = 0;
            ...
            foreach ($lines as $line) {
                ...
                if ($tm3!=0) {
        			...
        			//数据分段细化
                    if ($tm4-$tm3<=60000) {//1minutes
                        $cntPie_tm3_4_1 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_1_new += 1;
            			}
        			}
        			else if ($tm4-$tm3<=180000) {//3minutes
        			    $cntPie_tm3_4_3 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_3_new += 1;
            			}
        			}
                    else if ($tm4-$tm3<=300000) {//5minutes
                        $cntPie_tm3_4_5 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_5_new += 1;
            			}
        			}
                    else if ($tm4-$tm3<=600000) {//10minutes
                        $cntPie_tm3_4_10 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_10_new += 1;
            			}
        			}
                    else if ($tm4-$tm3<=900000) {//15minutes
                        $cntPie_tm3_4_15 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_15_new += 1;
            			}
        			}
                    else if ($tm4-$tm3<=1800000) {//30minutes
                        $cntPie_tm3_4_30 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_30_new += 1;
            			}
        			}
                    else if ($tm4-$tm3<=3600000) {//60minutes
                        $cntPie_tm3_4_60 += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_60_new += 1;
            			}
        			}
                    else {
                        $cntPie_tm3_4_oth += 1;
                        if ($isNew == 1) {
            			    $cntPie_tm3_4_oth_new += 1;
            			}
        			}
                }
            }
            ...
            $flashLoadLogData = self::_getFlashLoadLog($dt, $dir);

            $dal = Hapyfish2_Stat_Dal_CloadTm::getDefaultInstance();
            ...
            $info['pie_tm3_4'] = json_encode(array(1=>$cntPie_tm3_4_1, 3=>$cntPie_tm3_4_3, 5=>$cntPie_tm3_4_5, 10=>$cntPie_tm3_4_10, 15=>$cntPie_tm3_4_15, 30=>$cntPie_tm3_4_30, 60=>$cntPie_tm3_4_60, 'oth'=>$cntPie_tm3_4_oth));
            ...
            $info['pie_tm3_4_new'] = json_encode(array(1=>$cntPie_tm3_4_1_new, 3=>$cntPie_tm3_4_3_new, 5=>$cntPie_tm3_4_5_new, 10=>$cntPie_tm3_4_10_new, 15=>$cntPie_tm3_4_15_new, 30=>$cntPie_tm3_4_30_new, 60=>$cntPie_tm3_4_60_new, 'oth'=>$cntPie_tm3_4_oth_new));
            ...
            if ($flashLoadLogData) {
                $info['flashloadlog'] = json_encode($flashLoadLogData['flashloadlog']);
                $info['flashloadlog_new'] = json_encode($flashLoadLogData['flashloadlog_new']);
            }
    ...
    private static function _getFlashLoadLog($dt, $dir)
    {
        //$strDate = date('Ymd', $dt);
        $retData = array();
        $strDate = $dt;
        //$fileName = '/home/admin/logs/weibo/stat-data/nocookie/' . $strDate . '/all-nocookie-' . $strDate . '.log';
        $fileName = $dir . $strDate . '/all-flashloadlog-' . $strDate . '.log';
        try {
            //file not exists
            if (!file_exists($fileName)) {
                info_log($fileName . ' not exists!', 'stat_DaycLoadTm');
            ...
            foreach ($lines as $line) {
                ...
                if ( $isNew == 1 ) {
	                if ( isset($flashloadlogNew[$step]) ) {
	                	$flashloadlogNew[$step]++;
	                }
	                else {
	                	$flashloadlogNew[$step] = 0;
	                }
                }
            }
            
            $retData['flashloadlog'] = $flashloadlog;
            $retData['flashloadlog_new'] = $flashloadlogNew;
            
        }
        catch (Exception $e) {
            info_log('_getFlashLoadLog:'.$e->getMessage(), 'stat_DaycLoadTm');
            return false;
        }
        return $retData;
    }
