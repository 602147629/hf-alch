package happyfish.cacher 
{
	...
	public class SwfClassCache extends EventDispatcher implements IClassManager
	{
		...
		private var domainList:Object = new Object;
		...
		private function loadClassSwf(url:String):void
		{
			...
			loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, loadClassSwf_complete);
		...
		private function loadClassSwf_complete(e:Event):void 
		{
			...
			var arr:Array = currentClass.split(".");
			domainList[arr[0] + arr[1]] = (e.target as LoaderInfo).applicationDomain;
		...
		public function hasClass(className:String):Boolean {
			if (appDomain.hasDefinition(className)) return true;
			
			var arr:Array = className.split(".");
			var domainName:String = arr[0] + arr[1];
			var theDomain:ApplicationDomain = domainList[domainName];
			if (!theDomain) return false;
			return theDomain.hasDefinition(className);
		}
		...
		public function getClass(className:String):Class {
			...
			else
			{
				...
				var domainName:String = arr[0] + arr[1];
				var theDomain:ApplicationDomain = domainList[domainName];
				if (!theDomain) return null;
				else if (theDomain.hasDefinition(className)) return theDomain.getDefinition(className) as Class;
			}
			return null;
		}
