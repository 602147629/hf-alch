package happyfish.display.ui 
{
	...
	public class GridPage extends EventDispatcher
	{
		...
		/**
		 * 选择后的回调函数 function (e:GridPageEvent):void;
		 */
		...
		private var _pagination:Pagination;//页码组件
		...
		public function GridPage(ui:Sprite,_container:DisplayObjectContainer,_hideButton:Boolean=true ) 
		{
			...
			if(iview != container) container.addChild(iview);
			
			iview.addEventListener(MouseEvent.CLICK, clickFun);
			
			iview.addEventListener(GridPageEvent.ITEM_SELECT, itemSelectFun);
			
		}
		
		public function set pagination(value:Pagination):void {
			_pagination = value;
			if(_pagination){ iview.addChild(_pagination)};
		}
		...
		{
			var hasGrid:Boolean = grid != null;
			if (hasGrid)
			{
				grid.clearAll();
				iview.removeChild(grid);
			}
		...
		/**
		 * 设置数据,并依key保留数据
		 * @param	value			数据组 Array or Vector
		 * @param	key				数据过滤依据字段
		 * @param	keyV			key为该值的数据保留
		 * @param	saveCurrentPage	是否还停在当前页页
		 */
		public function setData(value:*, key:String = "", keyV:*= null, saveCurrentPage:Boolean = false):void {
			if (!value) 
			{
				return;
			}
			totalData = (value is Array) ? value : VectorUtil.toArray(value);
			filterKey = key;
			filterValue = keyV;
			...
			if (pagination) {
				pagination.setData(currentPage-1, Math.ceil(data.length/pageLength)-1);
			}
		...
		public function initPage():void {
			...
			if (iview.getChildByName("pageNumTxt")) iview["pageNumTxt"].text = currentPage+"/" + Math.ceil(data.length / pageLength);
			...
			for (var i:int = 0; i < pageLength; i++) 
			{
				if (data[i+addNum]) 
				{
					tmp = createItem(data[i + addNum],i+addNum);
					
					grid.add(tmp);
				}else {
					tmp = createBlankItem(i+addNum);
			...
			if (pagination) 
			{
				pagination.setData(currentPage-1);
			}
		}
		...
		public function createBlankItem(index:int):GridItem
		{
			return null;
		}
		
		public function getItemByValue(value:*):GridItem
		{
			var idx:int = data.indexOf(value);
			if ( -1 == idx) return null;
			idx -= pageLength * currentPage-pageLength;
			if (idx >= 0 && idx < pageLength) return grid.data[idx] as GridItem;
			return null;
		}
		 ...
		 * @param	reDraw
		 */
		public function removeItem(cid:*,num:uint=1, reDraw:Boolean = true):void { 
		...
		protected function setBtnVisible(target:DisplayObject,value:Boolean):void {
			if (target) 
			{
				if (hideButtonFlag) 
				{
		...
		protected function createItem(value:Object,index:int):GridItem {
		...
		public function clear():void {
			...
			if (iview != null)
			{
				...
				iview.removeEventListener(GridPageEvent.ITEM_SELECT, itemSelectFun);
			}
		}
