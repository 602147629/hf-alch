package happymagic.manager 
{
	...
	public class DataManager 
	{
		...
		public function getSceneVoByClass(id:uint,state:uint):SceneVo {
			for (var i:int = 0; i < sceneClass.length; i++) 
			{
				if (id == sceneClass[i].sceneId)
				{
					...
					var obj:Object = BasicVo.toObject(sceneClass[i]);
					for (var name:String in obj) sceneVo[name] = obj[name];
					return sceneVo;
				}
			}
		...
		public function getLocalFogData(sceneId:int = -1):ByteArray
		{
			...
			if (fogData[sid] == null)
			{
				...
				fogData[sid] = FogManager.getEmptySceneData(sceneClassVo.numCols, sceneClassVo.numRows);
				localData.flush();
			}
			return  fogData[sid] as ByteArray;
		}
	}
