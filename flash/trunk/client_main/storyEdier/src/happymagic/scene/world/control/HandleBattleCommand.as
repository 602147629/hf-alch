package happymagic.scene.world.control 
{
	...
	public class HandleBattleCommand 
	{
		...
		private function battleInit(event:Event):void
		{
			event.target.removeEventListener(Event.COMPLETE, battleInit);
			
			var command:BattleInitCommand = event.target as BattleInitCommand;
			
			var fightBg:String = (monster.vo as MonsterVo).fightBg;
			if (fightBg == null || fightBg == ""){
				var sceneClassVo:SceneClassVo = DataManager.getInstance().getCurrentScene();
				fightBg = sceneClassVo.fightBg;
			}
			if(fightBg != null) command.objdata["BattleVo"]["bgClassName"] = fightBg;
			
			DataManager.getInstance().setVar("battleInitData", command.objdata);
