package happymagic.display.control 
{
	...
	public class BuyItemMsgControl 
	{
		...
		public function init(type:uint):void
		{
			...
			{
				case MoneyType.GEM:
			           DisplayManager.showSysMsg(LocaleWords.getInstance().getWord("buyitemmsggem"), SysMsgView.TYPE_CONFIRM);
					   EventManager.getInstance().addEventListener(SysMsgEvent.SYSMSG_CLOSED, gotorecharge);
				break;
				
				case MoneyType.COIN:
					DisplayManager.showSysMsg(LocaleWords.getInstance().getWord("buyitemmsgcoin"), SysMsgView.TYPE_MSG);				    
				break;
			}
