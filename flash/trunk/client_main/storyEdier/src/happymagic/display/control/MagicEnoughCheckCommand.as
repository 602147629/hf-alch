package happymagic.display.control 
{
	...
	public class MagicEnoughCheckCommand 
	{
		 ...
		 * @return	够为true
		 */
		public function check(sp:int):Boolean {
			if (DataManager.getInstance().getEnoughSp(sp)) 
			{
				return true;
			}else {
				init();
		...
		{
			DisplayManager.showSysMsg(LocaleWords.getInstance().getWord("teachMagicNoEnough"), SysMsgView.TYPE_CONFIRM);
