package happymagic.display.control 
{
	...
	public class StoryPlayCommand 
	{
		...
		private var storyEndEvent:DEvent;
		...
		private function startPlay():void
		{
			...
			EventManager.getInstance().dispatchEvent(new DEvent(STORY_BEGIN,story.id));
		...
		{
			killed = true;
			CameraControl.getInstance().stopFollow();
			storyEndEvent = new DEvent(STORY_END, story.id);
			
			if (story.nextStoryId != 0) //连续触发
			{
				new ShowStoryCommand(story.nextStoryId);
				clear();
				EventManager.getInstance().dispatchEvent(storyEndEvent);
			}
			else{
				showAward(); //显示奖励
		...
		private function closeMovieMask_complete(e:Event):void 
		{
			...
			backToTheOriWorld(); //返回真实的世界
			
			if (story.battleData != null) //需要触发战斗
			{
				HandleBattleCommand.getInstance().battleInit(null, story.battleData);
			}
		...
		public static const STORY_BEGIN:String = "stroyBegin";
		public static const STORY_END:String = "storyEnd";
	}
