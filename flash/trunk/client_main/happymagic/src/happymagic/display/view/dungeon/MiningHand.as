package happymagic.display.view.dungeon 
{
	 ...
	 * @author XiaJunJie
	 */
	public class MiningHand extends Sprite
	{
		...
		public function update(cid:int, num:int = -1, red:Boolean = false):void
		{
			...
			if (num == -1) numTxt.visible = false;
			else
			{
				numTxt.visible = true;
				numTxt.text = "x" + num;
				numTxt.textColor = red ? 0xDD0000 : 0xDDDDDD;
			}
		}
		...
		private function classGeted(e:SwfClassCacheEvent):void 
		{
			if (e.className==loadingItem.className) 
			{
				...
				if (currentItemClassVo.cid == loadingItem.cid)
				{
					...
					if (tmpclass) icon = new tmpclass() as MovieClip;
					else
					{
						icon = new MovieClip;
						with (icon.graphics)
