package happymagic.display.view.mainMenu 
{
	...
	public class MainMenuView extends UISprite
	{
		...
		public function MainMenuView() 
		{
			...
			leftBtns_selfHome = [_iview.roleInfoBtn,_iview.illusBtn, _iview.itemboxBtn, _iview.shopBtn, _iview.friendBtn,_iview.roleWorkBtn,_iview.diyBtn];
			leftBtns_friend = [_iview.roleInfoBtn, _iview.itemboxBtn, _iview.shopBtn, _iview.friendBtn];
			leftBtns_selfViliage = [_iview.roleInfoBtn, _iview.illusBtn, _iview.itemboxBtn, _iview.shopBtn, _iview.friendBtn, _iview.hireBtn,_iview.roleWorkBtn];
		...
		private function sortBtn(arr:Array):void {
			...
			TweenMax.to(_iview.bg, .5, { width:len * 56 + 10 } );
			TweenMax.to(_iview.configBtn, .5, { x:len * 57 + 25 } );
			
			_iview.unFullSceenBtn.visible = (DisplayManager.uiSprite.stage.displayState == StageDisplayState.FULL_SCREEN);
			_iview.fullSceenBtn.visible = !(DisplayManager.uiSprite.stage.displayState == StageDisplayState.FULL_SCREEN);
			
			TweenMax.to(_iview.unFullSceenBtn, .5, { x:arr.length * 57 + 55 } );
			TweenMax.to(_iview.fullSceenBtn, .5, { x:arr.length * 57 + 55 } );
		...
		private function clickFun(e:MouseEvent):void 
		{
			...
			case _iview.roleWorkBtn:
				ActModuleManager.getInstance().addActModule(DataManager.getInstance().getActByName("RoleWork"));
				break;
				
				case _iview.fullSceenBtn:
					_iview.stage.displayState = StageDisplayState.FULL_SCREEN;
					fullScreen = true;
				break;
				
				case _iview.unFullSceenBtn:
					_iview.stage.displayState = StageDisplayState.NORMAL;
					fullScreen = false;
				break;
