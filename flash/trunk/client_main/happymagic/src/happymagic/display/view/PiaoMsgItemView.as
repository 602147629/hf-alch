package happymagic.display.view 
{
	...
	public class PiaoMsgItemView extends Sprite
	{
		...
		public function setData(_type:uint, _value:*, _toX:int, _toY:int,_delay:uint=0, _strType:String = null, _icon:DisplayObject = null, _txtMc:MovieClip = null, completeFun:Function = null):void {
			...
			if (!justShow && type!=PiaoMsgType.TYPE_BAD_STRING && type!=PiaoMsgType.TYPE_GOOD_STRING) 
			{
				...
				toPoint ||= PiaoMsgControl.getInstance().getPoint(_strType);
				if(toPoint) toPoint = parent.globalToLocal(toPoint);
			}
			...
			case PiaoMsgType.TYPE_OTHER:
					icon = _icon;
					txtMc = _txtMc || new piaoNumberTxt();
					setTxtAutoSize();
					tmpf = txtMc.txt.getTextFormat();
					tmpf.letterSpacing = 1;
					txtMc.txt.htmlText = HtmlTextTools.fontWord(value.toString(),"#339900");
			...
			case PiaoMsgType.TYPE_ITEM:
				    var itemvo:BaseItemClassVo = DataManager.getInstance().itemData.getItemClass(int((value as ConditionVo).id));
					var iconclass:Class = SwfClassCache.getInstance().getClass(itemvo.className);				
					icon = new iconclass();
					icon.scaleX = 0.6;
					icon.scaleY = 0.6;					
					txtMc = new piaoExpTxt();
					...
					tmpf = txtMc.txt.getTextFormat();
					tmpf.color = 0xFFCC00;
					txtMc.txt.text = (value.num > 0 ? "+ " : "") + (value as ConditionVo).num.toString();
					txtMc.txt.setTextFormat(tmpf);					
				break;
				
			    case PiaoMsgType.TYPE_BATTLE_EXP:
					icon = new awardIcon_4();
					icon.scaleX = 0.8;
					icon.scaleY = 0.8;
					txtMc = new piaoExpTxt();
					txtMc.y += 25;
