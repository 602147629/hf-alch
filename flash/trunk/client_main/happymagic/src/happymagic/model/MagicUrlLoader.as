package happymagic.model 
{
	...
	public class MagicUrlLoader extends UrlConnecter
	{
		...
		public static var showSysMsg:Function;
		...
		override public function load_complete(e:Event):void 
		{
			...
			if ((e.target as URLLoader).data) 
			{
				if (compress) 
				{
					var tmpByteArr:ByteArray = e.target.data as ByteArray; 
					tmpByteArr.uncompress();
					data = tmpByteArr.toString();
				}else {
					data = e.target.data;
				}
			...
			if (data == "-100" || data == "系统繁忙或服务器错误，请稍后再试。" || data == "Server Error(-100)" )
			{
				...
				//显示错误弹出窗
				if (takeError && showSysMsg != null) 
				{
					showSysMsg(LocaleWords.getInstance().getWord("UrlConnectError"));
				}
			}else {
				dispatchEvent(e);
			}
