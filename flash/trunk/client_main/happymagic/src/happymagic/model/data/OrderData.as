package happymagic.model.data 
{
	...
	public class OrderData 
	{
		...
		public function getOrderList(other:Boolean = false, hasAccept:Boolean = true, hasUnaccept:Boolean = true):Vector.<OrderVo>
		{
			var list:Vector.<OrderVo> = other ? otherOderList : myOrderList;
			if (hasAccept && hasUnaccept) return list;
			
			var arr:Vector.<OrderVo> = new Vector.<OrderVo>();
			var len:int = list.length;
			for (var i:int = 0; i < len; i++)
			{
				if ((hasAccept && list[i].state != OrderType.REQUEST) ||
					(hasUnaccept && list[i].state == OrderType.REQUEST))
				{
					arr.push(list[i]);
				}
			}
