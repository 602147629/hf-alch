package happymagic.model.data 
{
	...
	public class ItemData 
	{
		...
		private var  _autoUseStaticList:Array = new Array();
		private var  _autoUseInitList:Array = new Array();
		/**
		 * 获取一个基础数据
		 * @param	cid
		 * @return
		...
		/**
		 * 根据标签返回道具
		 * @param	souceArr 源物品数组
		 * @param	labels 	标签数组
		 * @return labels为null或空数组时返回空数组
		 */
		public function getItemListByLabel(souceArr:Array,labels:Array):Array
		{
			if (!labels || 0 == labels.length) return [];
			
			var v:Array = [];
			var mapLen:int = souceArr.length;
			for (var i:int = 0; i < mapLen; i++)
			{
				var vo:ItemVo = ItemVo(souceArr[i]);
				var map:Object = vo.base.labelMap;
				for (var k:int = labels.length - 1; k >= 0; k--)
				{
					if (map[labels[k]])
					{
						v.push(vo);
		...
		public function get autoUseStaticList():Array 
		{
			return _autoUseStaticList;
		}
		
		public function set autoUseStaticList(value:Array):void 
		{
			_autoUseStaticList = value;
		}
		
		public function get autoUseInitList():Array 
		{
			return _autoUseInitList;
		}
		
		public function set autoUseInitList(value:Array):void 
		{
			_autoUseInitList = value;
		}
		
		//这个有没有特殊功能
		public function isSpecialItemVo(cid:int):Boolean
		{
			for (var i:int = 0; i < autoUseStaticList.length; i++) 
			{
				if (autoUseStaticList[i] == cid)
				{
					return true;
				}
			} 
		...
		//这个特殊的道具是不是开启		
		public function isSpecialOpen(cid:int):Boolean
		{
			for (var i:int = 0; i < _autoUseInitList.length; i++) 
			{
				if (_autoUseInitList[i] == cid)
				{
					return true;
				}				
			}
