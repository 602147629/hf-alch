package happyfish.actModule.giftGetAct.view.myWish 
{
	...
	public class MyWishView extends MyWishViewUi
	{
		...
		public function MyWishView() 
		{
			...
			//{
                  //mywish[i].className = GiftDomain.getInstance().getGiftDiaryVoClassName(mywish[i].type, id);
                  //mywish[i].name = GiftDomain.getInstance().getGiftDiaryVoName(mywish[i].type, id);			  
			//}
		...
		private function releaseMyWishCommondComplete(e:Event):void 
		{
			...
			if (e.target.data.result.isSuccess)
			{
				...
				str += LocaleWords.getInstance().getWord("GiftGetActword13") + friendIdList.length + LocaleWords.getInstance().getWord("GiftGetActword14");
