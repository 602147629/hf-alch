package happyfish.actModule.giftGetAct.manager 
{
	...
	public class GiftDomain implements IGiftDomain
	{
		...
		{
		   DisplayManager.showSysMsg(msg, SysMsgView.TYPE_MSG);
		}
		
		//飘屏提示框（msg 是内容）
		public function showPiaoStr(str:String):void
		{
			DisplayManager.showPiaoStr(PiaoMsgType.TYPE_BAD_STRING, str);
		}
		...
		//根据名字获取接口地址
		public function getInterfaceUrl(name:String,_withtime:Boolean = true):String
		{
			
			return InterfaceURLManager.getInstance().getUrl(name,_withtime);
		...
		public function showAwardView(data:Object):void
		{
		    ...
		    if (data.addItems)
				{
					for (i = 0; i < data.addItems.length; i++) 
					{
						
						awards.push(new ConditionVo().setData( { type:ConditionType.ITEM, id:data.addItems[i].cid, num:data.addItems[i].num } ));
					}
				}
			...
			//{
				...
				//{
					//awards.push(new ConditionVo().setData( { type:ConditionType.ITEM, id:data.addItem[i][0], num:data.addItem[i][1] } ));
				//}
			//}				
				...
				awardwin.setData( { name:LocaleWords.getInstance().getWord("GiftGetActword1"), awards:awards } );
			...
			public function getGiftDiaryVoName(_type:uint, _giftCid:uint):String
			{
				...
				{
					case 1:
					    return DataManager.getInstance().itemData.getItemClass(_giftCid).name;							
					break;
			...
			public function getGiftDiaryVoClassName(_type:uint, _giftCid:uint):String
			{
				...
				{
					case 1:
		          		  return DataManager.getInstance().itemData.getItemClass(_giftCid).className;					    
					break;
