package happyfish.actModule.giftGetAct.view.receiveGift 
{
	...
	public class ReceiveGiftView extends ReceiveGiftViewUi
	{
		...
		public function init():void
		{
			...
			{
				 this.numbertips.visible = true;
				 var giftuservo:GiftUserVo = GiftDomain.getInstance().getVar("giftUserVo");
				 this.numbertips["num"].text = String(giftuservo.giftNum);
			}
			else
			{
				 this.numbertips.visible = false;				 
		...
		private function receivegiftcommondcomplete(e:Event):void 
		{
			...
			{
			       var str:String = LocaleWords.getInstance().getWord("GiftGetActword17") + giftdiaryId.length +LocaleWords.getInstance().getWord("GiftGetActword18");
