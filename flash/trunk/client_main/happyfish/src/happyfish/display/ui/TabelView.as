package happyfish.display.ui
{
	...
	public class TabelView extends Sprite
	{
		...
		private var autoSort:Boolean;
		
		public function TabelView(autoSort:Boolean = true) 
		{
		...
		public function setAbleItem(index:uint,flag:Boolean):void {
			...
			if (flag) 
			{
				...
				tmp.filters = [FiltersDomain.grayFilter];
				tmp.mouseChildren = 
		...
		{
			var tmp:DisplayObject = e.target as DisplayObject;
			if (this == tmp) return;
			
			while (tmp.parent != this)
			{
				tmp = tmp.parent;
			}
			
			select(MovieClip(tmp).index);
		}
