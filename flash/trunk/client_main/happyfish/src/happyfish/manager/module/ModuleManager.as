	...
	public class ModuleManager extends EventDispatcher
	{
		...
		private function moudleShowMv(module:ModuleVo):void {
			...
			switch (module.mvType) 
			{
				...
				tmpmd.view.visible = true;
				tmpmd.view.alpha = 1;
				//TweenMax.from(tmpmd.view, module.mvTime, { scaleX:0, scaleY:0, immediateRender:false, ease:Back.easeOut,onComplete:moduleShowComplete,onCompleteParams:[tmpmd]} );
				TweenMax.from(tmpmd.view, module.mvTime, { x:"-300", autoAlpha:0, immediateRender:false, ease:Cubic.easeOut, onComplete:moduleShowComplete, onCompleteParams:[tmpmd] } );
				break;
		...
		public function closeModule(moduleName:String,del:Boolean=false):void {
			if (modules[moduleName]) 
			{
				...
				{
					case ModuleMvType.NONE:
					closeModule_complete(moduleName,tmpmd, del);
					
					break;
					
					case ModuleMvType.CNETER:
					TweenMax.to(tmpmd.view, moduleVo.mvTime, { x:"+300",autoAlpha:0,onComplete:closeModule_complete,onCompleteParams:[moduleName,tmpmd,del], ease:Cubic.easeOut } );
					break;
					
					case ModuleMvType.FROM_BOTTOM:
					TweenMax.to(tmpmd.view, moduleVo.mvTime, { y:toP.y, onComplete:closeModule_complete, onCompleteParams:[moduleName,tmpmd, del] } );
					break;
					case ModuleMvType.FROM_TOP:
					TweenMax.to(tmpmd.view, moduleVo.mvTime, { y:toP.y, onComplete:closeModule_complete, onCompleteParams:[moduleName,tmpmd, del] } );
					break;
					...
					case ModuleMvType.FROM_LEFT:
					TweenMax.to(tmpmd.view, moduleVo.mvTime, { x:toP.x, onComplete:closeModule_complete, onCompleteParams:[moduleName,tmpmd, del] } );
					break;
					case ModuleMvType.FROM_RIGHT:
					TweenMax.to(tmpmd.view, moduleVo.mvTime, { x:toP.x, onComplete:closeModule_complete, onCompleteParams:[moduleName,tmpmd, del] } );
					break;
		...
		private function closeModule_complete(moduleName:String,module:IModule,del:Boolean):void
		{
			...
			if (module) 
			{
				...
				module.view.visible = false;
				module.scaleX=
				module.scaleY = 1;
				
				//如果关闭的是当前单独窗口
				if (curSingleModule==module) 
				{
					//显示下一个等待窗口
				...
				//清除等待显示单独窗口
				clearWaitModule(module);
				
				if (del) 
				{
					module.view.parent.removeChild(module.view);
				}
			}
			...
			if (module) 
			{
				module["dispatchEvent"](new Event(Event.CLOSE));
			}
		}
		...
		{
			setTimeout(alginAllModule, 0);
		}
		
		private function alginAllModule():void {
		...
		private function alginModule(moduleName:String, tween:Boolean = true):void {
			...
			var boundX:Number=0;
			var boundY:Number=0;
			if (setting.useBounds) 
			{
				boundY = - mrect.height / 2 - mSelfRect.y;
				boundX = - mrect.width / 2 - mSelfRect.x;
			}
