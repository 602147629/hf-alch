package happyfish.scene.world 
{
	...
	public class WorldState
	{
		 ...
		 * @param	tileZ	房间的Z大小
		 */
		public function initGrid(tileX:int, tileZ:int,sceneVo:SceneVo):void
		{
			
			//创建grid
			//this.grid = new Grid(tileX, tileZ);
			this.grid = new Grid(sceneVo.numCols, sceneVo.numRows); //2011.11.18
			...
			if (tileX==0 || tileZ==0) 
			{
				...
				roomRect = new Rectangle(IsoUtil.roomStart, IsoUtil.roomStart, tileX + 1, tileZ + 1);
				
				wallRectLeft = new Rectangle(IsoUtil.roomStart, IsoUtil.roomStart, tileX+1, 1);
				wallRectRight = new Rectangle(IsoUtil.roomStart, IsoUtil.roomStart, 1, tileZ + 1);
				
				doorRectLeft = new Rectangle(IsoUtil.roomStart + 1, IsoUtil.roomStart + 1, tileX + 1, 1);
				doorRectRight = new Rectangle(IsoUtil.roomStart + 1, IsoUtil.roomStart + 1, 1, tileZ + 1);
			}
