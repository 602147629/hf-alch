package happyfish.scene.world.grid 
{
	...
	public class IsoItem extends BaseItem
	{
		...
		public var block:Boolean = true; //是否挡路
		public var nodes:Object = new Object;
		private var mouseState:String="normal";
		
		private var nameTxt:TextField;
		
		/**
		 * 
		 * @param	$data	[Object] className,x,z
		 * @param	$worldState
		 */
		...
		public function showName(str:String):void {
			if (!nameTxt) 
			{
				...
				var glow:GlowFilter = new GlowFilter(0x000000, 1, 2, 2, 10, 1);
				nameTxt.filters = [glow];
			}
		...
		public function set mouseEvent(value:Boolean):void
		{
			view.container.mouseEnabled = view.container.mouseChildren = value;
			...
			{
				view.container.removeEventListener(MouseEvent.ROLL_OVER, this.onMouseOver);
				view.container.removeEventListener(MouseEvent.ROLL_OUT, this.onMouseOut);
				view.container.removeEventListener(MouseEvent.MOUSE_MOVE, this.onMouseOverMove);
				view.container.removeEventListener(MouseEvent.CLICK, this.onClick);
			}
		}
		...
		public function resetView(className:String="",_callBack:Function=null):void {
			...
			newasset.className = _data.className;
        ...
        override public function remove() : void
        {
			...
			//从层中清除
            if(_view.parent) this._view.parent.removeIsoChild(this._view);
			//标记已清除
            this.alive = false;
			//清除自己的印章对象
		...
		protected function onMouseOverMove(e:MouseEvent):void
		{
			...
			if (!asset.bitmap_movie_mc.bitmapData) return;
		...
		protected function onClick(e:MouseEvent):void
		{
			...
			if (alpha>0) 
			{
				...
				if (view.container == e.target && mouseNotInAsset(e.stageX, e.stageY)) {
					view.container.dispatchEvent(new GameMouseEvent(GameMouseEvent.CLICK, this, typeName, e));
				}else {
		...
		private function mouseNotInAsset(stageX:Number, stageY:Number):Boolean
		{
			var arr:Array = view.container.getObjectsUnderPoint(new Point(stageX, stageY));
			for (var i:int = arr.length -1; i >= 0; i--)
			{
				if (!asset.contains(DisplayObject(arr[i]))) return true;
			}
			return false;
		}
		...
		public function rorate($flag:Boolean = true):void
		{
			...
			if ($flag) {
				if (this.asset.scaleX == -1) {
					this.grid_size_x = this._data.sizeZ;
					this.grid_size_z = this._data.sizeX;
				}
				
				if (this.asset.scaleX == 1) {
					this.grid_size_x = this._data.sizeX;
					this.grid_size_z = this._data.sizeZ;
				}
			} else {
				if (this.asset.scaleX == 1) {
					this.grid_size_x = this._data.sizeZ;
		...
		public function move($grid_pos:Point3D):void
		{
			...
			//var uiDiyMenu:Sprite = PublicDomain.getInstance().getVar("uiDiyMenu") as Sprite;
			/*var uiSprite:Sprite = PublicDomain.getInstance().getVar("uiSprite") as Sprite;
			if (uiSprite.hitTestPoint(this._view.container.stage.mouseX, this._view.container.stage.mouseY, true)) {
				if (!this.isoUiSprite) {
					this.isoUiSprite = new CacheSprite();
					this.isoUiSprite.className = this._data.className;
					uiSprite.addChild(this.isoUiSprite);
					this.isoUiSprite.mouseChildren = 
					this.isoUiSprite.mouseEnabled = false;
				}
				
				this.isoUiSprite.x = uiSprite.mouseX;
				this.isoUiSprite.y = uiSprite.mouseY;
		...
		{
			if (this.isoUiSprite) {
				(PublicDomain.getInstance().getVar("uiSprite") as Sprite).removeChild(this.isoUiSprite);
