package happyfish.utils.display 
{
	...
	public class ItemDropController extends EventDispatcher 
	{
		...
		private var dataList:Vector.<ConditionVo>;
		private var _isDroping:Boolean;
		private var remain:int;
		private var list:Vector.<ConditionVo>;
		 ...
		 * @param	w : icon的宽
		 * @param	h : icon的高
		 * @param   _isfly:这堆物品或者金币是否要飞上去 true:飞 false:不飞
		 */
		public function drop(list:Vector.<ConditionVo>, from:Point, to:*, w:int = 30, h:int = 30,_isfly:Boolean = false):void
		{
			if (list.length == 0)
			{
			...
			dataList = new Vector.<ConditionVo>();
			for each(var conditionVo:ConditionVo in list)
			{
				...
				if (conditionVo.type==ConditionType.ITEM) //物品
				{
					...
					for (var i:int = 0; i < conditionVo.num; i++) create(className,conditionVo);
				}
				else if (conditionVo.type == ConditionType.USER)
				{
					...
					else if (conditionVo.id == ConditionType.USER_BATTLE_EXP) className += "4";
					
					create(className,conditionVo);
				}
			}
		...
		private function create(className:String,conditionVo:ConditionVo):void
		{
			//创建图标
			var iconView:IconView = new IconView(w, h);
			iconView.setData(className);
			iconView.buttonMode = true;
			container.addChild(iconView);
			iconView.x = from.x;
			iconView.y = from.y;
			
			iconList.push(iconView);
			if (conditionVo.type == ConditionType.USER)
			{
			...
			dataList.push(conditionVo);
			//计算目标点
			var target:Point = new Point;
			target.x = from.x + (Math.random() - 0.5) * 200;
			target.y = from.y + (Math.random() - 0.5) * 200;
		...
		private function onComplete(event:Event):void
		{
			...
			if(needToGotoTarget) (event.target as MoveController).object.addEventListener(MouseEvent.MOUSE_OVER, onIconRollOver);
		...
		{
			event.target.removeEventListener(MouseEvent.MOUSE_OVER, onIconRollOver);
		...
		private function gotoTarget(iconView:IconView):void
		{
            ...
            var condvo:ConditionVo = dataList[index];
			...
			var mainview:MainInfoView = ModuleManager.getInstance().getModule("mainInfo") as MainInfoView;
			
			if (condvo.type == ConditionType.ITEM)
			{
				if (menuview.state==ModuleStateType.SHOWING&&isfly) 
				{
					tmpP = menuview.getBtnPoint("itemboxBtn");
					tmpP = container.globalToLocal(tmpP);					
					TweenLite.to(iconView, 0.1, { x:iconView.x, y:iconView.y, onComplete:onGotoTargetComplete, onCompleteParams:[iconView] } );
				...
				tmpP.x = iconView.x;
				tmpP.y = iconView.y;				
				tmpP = container.localToGlobal(tmpP);
				
				DisplayManager.showPiaoItemStr(PiaoMsgType.TYPE_ITEM, condvo, tmpP, true, !isfly);
			}
			else if (condvo.type == ConditionType.USER)
			{
                tmpP = container.localToGlobal(tmpP);
				
				if (condvo.id == ConditionType.USER_COIN)
				{
					msgtype = PiaoMsgType.TYPE_COIN;
				}
				else if(condvo.id == ConditionType.USER_EXP)
				{
					msgtype = PiaoMsgType.TYPE_EXP;
				}
				else if (condvo.id == ConditionType.USER_GEM)
				{
					msgtype = PiaoMsgType.TYPE_GEM;
				}
				
                if (condvo.id == ConditionType.USER_BATTLE_EXP)
				{
					msgtype = PiaoMsgType.TYPE_BATTLE_EXP;
				}
				else
				{
                    
				}
				DisplayManager.showPiaoStr(msgtype, condvo.num.toString(), tmpP, true, !isfly);	
